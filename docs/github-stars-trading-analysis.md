...
| 级别 | 事项 | 来源 | 工作量 |
|---|---|---|---|
| **P0** | 回测正确性：接 ths 公司行动+复权因子（当前未复权=跨除权日信号错误） | Financial-API 盘点 | 1 天 |
| **P0** | 预判模块接 ths 集合竞价端点（D1 竞价验证自动化） | Financial-API 盘点 | 0.5 天 |
| **P1** | easy_tdx 试点：实测 TDX 分钟历史深度+一周稳定性 → 达标则作分钟历史专用源扩回测窗口（22 天→数月），并评估逐笔历史接入 | easy_tdx | 实测 0.5 天 + 接入 1.5 天 |
