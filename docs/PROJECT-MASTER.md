...
| 4 投研数据 | 龙虎榜总览+席位+历史/资金流/财务/估值/公司资料/公告/新闻 + **新闻·公告摘要 v1**（规则摘要器 + LLM 可插拔降级） | ✅；余：营业部关系图谱、筹码、解禁、两融、大宗 |
| 5 量化系统 | 多因子技术评估(MA/MACD/KDJ/RSI/量价/形态) + 全市场选股器（截面过滤→TDX日K→六维评分卡，防飞刀修正） + **风控引擎 v1**（7 档市场状态→仓位建议参数→下单预检） | 🔶；余：更多副图 |
