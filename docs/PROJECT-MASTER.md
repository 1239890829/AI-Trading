...
## 1.2 当前状态快照
- 后端：FastAPI（Python 3.11），81 个 REST 端点 + 1 个 WebSocket，**四源 Provider 链**（ths→tencent→eastmoney→sina）+ mock
...
# 五、REST API 全表（81 端点，**完整清单与鉴权说明见 docs/api.md**，此处保留增量与要点）
...
| 2 行情基础设施 | Provider协议/四源链/Normalizer/5级质量/QuoteHub/REST 81端点/WS/指数/个股/K线/分时/盘口/逐笔 + **数据可靠性**（Parquet 原子写/容错读、涨跌停价补全共享化、统一缓存层 ttl_cache） | ✅ 全部 |
...
| 7 AI 系统 | 盘后复盘 Agent + 新题材预判 + 新闻摘要 v1（均为规则层） | 🔶；规则层已完成，余：LLM 接入/四角色编排（阻塞于凭据）、MCP 封装 |
| 8 通知与部署 | 预警规则/触发/通知通道抽象/管理页 + 同源反代 + 运维巡检工具(api-sweep) | 🔶；余：真实推送通道、Docker 生产化、监控（阻塞于部署决策） |
