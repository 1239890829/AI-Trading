...
| 2 行情基础设施 | Provider协议/四源链/Normalizer/5级质量/QuoteHub/REST 端点（**数量以 `/openapi.json` 为权威**，见 AGENTS §2）/WS/指数/个股/K线/分时/盘口/逐笔 + **数据可靠性**（Parquet 原子写/容错读、涨跌停价补全共享化、统一缓存层 ttl_cache、天梯 seal_nextday 源自证） | ✅ 全部 |
