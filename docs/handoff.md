...
> **定位 / 摘要（2026-09-26 当前）**：IMP-048 第三十片 PR #165 已合并为 `c94c3025d6cac3904fab9774e0044bc4db29ca8b`，合并后 master CI #36207908210 三项通过；第三十一片正在修正撤销事件仍可被手动补方向激活解释的缺口，发布仍以准确 HEAD 回执、CI 与合并后核验为准。阶段页记录本轮反例与剩余范围。用户已要求持续推进 IMP-048 直到任务完成；临时授权 Codex 免网页审核自动执行，直到要求“改回来”，阶段门、完整门禁与准确 HEAD 发布回执仍适用。
...
**IMP-048 当前开发现场（2026-09-26）**：第三十一片隔离复现：`resolved`/`rejected` 事件仍能被 `backfill_event` 写入新方向及 `active` 解释，手动补方向路由也扫描它们。现于读取前筛选活跃、可见、无待审事件，写事务内再次核状态；定向修前 3 失败、修后 3 通过。旧事件循环守卫对前片多行 `to_thread` 调用产生两项假红，已改为要求两处包裹均存在，并通过真实裸调用注入判红及哈希恢复；最终本机全后端 **4359 passed / 80 skipped / 0 failed**。第三十片 PR #165 已完成准确 HEAD 发布与 post-merge CI。旧库筛查队列 36 条已全部作者预标：22 条标题直接关联、2 条原文核实非直接关联、12 条未知；其中只有 3 条有原文核验，独立人工 gold 仍为 0。默认 `data/ashare.db` 仍为旧 schema `b5c9e7a2d4f1`，`backend/data/ashare.db` 是空文件，8000 端口无后端实例；无准确新版运行库验收或生产加载。IMP-048 仍为 `进行中`，剩余范围见 [W03/IMP-048](stages/w03-execution.md#imp-048)。
...
**IMP-048 传播核对**：本片只收紧存量事件补方向的状态/可见性及其写入守卫，不新增评分权重、交易动作或发布权限。所属 W03 stage 与本 handoff 记录反例、当前发布和运行边界；代码契约在 `backend/app/events/store.py` 与 `backend/app/api/routes/events.py`，事件循环守卫在既有测试中同步更新。总方案、领域蓝图、Jev 蓝图、plan-registry、INDEX、AGENTS 和 Skills 均无长期规则变化，故不适用；没有新增文档或第二任务账本。
