...
- **独立审核边界**：本段只是作者执行事实/自检，不是 U49 `Review`。PR #67 即使 required CI 全绿，缺独立网页 Review 回执时新版 `release_check.py` 必须保持 BLOCKED；不得由本实现会话补写 APPROVED 来绕过。
