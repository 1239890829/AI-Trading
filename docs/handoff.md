...
- **最新授权与当前施工**：用户允许合理使用剩余 Actions 额度并继续实施。PR #25 已合并为 `e976c10`（准确 HEAD `c0710a1`，三项 CI 全成功，实际树一致）；当前从该最新 master 创建 `codex/index-response-identity`（`/tmp/ashare-index-response`），补东财指数返回身份与缺项保护。
...
- **当前完整门禁（BUG-020 东财指数响应身份）**：后端 **3596 collect / 3520 passed / 76 skipped / 0 failed，164.42s**；前端本地/UTC **651 passed / 70 文件**，单 worker **75.75s / 88.18s**。8000/3000 停用、前后端全量串行；后端期间有隔离 ASGI 与旧工作区归档，负载抽样 3.15/3.28/3.34；前端期间有只读核查、文档回填和账单页面操作。tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
  `e976c10` 的 3578/218 文件 → 3596/219 文件，全部 **+18** 为新增东财 HTTP/Hub 回归；旧用例身份无删改，import-lint 269、skipped 76 不变。前端源码零差异，651 用例身份全部相同。OpenAPI **170 paths / 零差异**；实际隔离 HTTP Provider→Hub→ASGI 验收 401、完整/缺失/恢复均 200，逐指数 high→stale→high，原价与时间在缺项时保留；非真实上游或盘中 SLA 证明。本批尚待 PR 与 master CI。
...
- **东财指数响应身份切片（2026-09-17）**：旧实现按裸六位代码关联并为漏项生成空报价，覆盖 Hub 的旧价格且掩盖缺失。新实现只接收请求内完整市场/代码身份，返回原顺序真实行；不制造缺项，重复身份或行结构错误抛 ProviderError。18 条独立 HTTP/Hub 回归在旧实现上 15 failed / 3 passed，修后连同关联回归 73 passed；再次回退旧实现 15 failed / 3 passed、恢复 SHA-256 相同，完整本地门禁与隔离 ASGI 验收见 §1，待集中交付。
