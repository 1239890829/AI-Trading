...
> **定位 / 摘要（2026-09-25 当前）**：IMP-048 第二十片在 PR #155；准确发布状态以该 PR 的 HEAD 回执、发布检查和合并后 master CI 为准。阶段页记录本轮修复与剩余范围，下一刀须在最终 master 重算选择器。用户临时授权 Codex 免网页审核自动推进，直到要求“改回来”；阶段门、完整门禁与准确 HEAD 发布回执仍适用。
...
**IMP-048 当前开发现场（2026-09-25）**：第二十片 PR #155 修正盘前简报的事件批次部分失败。修前隔离复现：先处理有效事件，再遇旧数据坏行，简报同时写“事件读取失败”和前一行的分数/版本引用；现整批成功才发布四类事件证据，失败则保留 missing 且不让残余事件分数参与方向排序。作者 U49 检查了旧版未知、顺序、失败反馈、归档与涨停池独立证据；成功批次计分、引用及通知门不变。定向 **28 passed**、全后端 **4341 passed / 80 skipped / 0 failed**，前端常规/UTC 各 **717 passed**，全量 pyflakes、tsc、eslint、Next build、文档体检、卫生及公开仓扫描通过。最终合并证据以 PR #155 的准确 HEAD `DegradedRelease`、发布检查和 master CI 为准；本片未加载生产实例，IMP-048 仍为 `进行中`，剩余范围见 [W03/IMP-048](stages/w03-execution.md#imp-048)。
...
**IMP-048 传播核对**：本片修正 W03 已登记的盘前简报事件证据部分失败语义，不新增判断事实源、业务目标或发布权限。所属阶段状态与剩余范围已更新 W03，本 handoff 记录当前 PR 与运行边界；代码契约在 `backend/app/picks/morning_brief.py`。总方案、领域蓝图、Jev 蓝图、plan-registry、INDEX、AGENTS 和 Skills 均无长期规则变化，故不适用；没有新增文档或第二任务账本。
