...
> **定位 / 摘要（2026-09-25 当前）**：IMP-048 第十六片 PR #146 已合并为 `e7f48481c5f0c593c4465b74dbd55b3742e6c9e2`，准确合并提交的 master CI `36024606016` 三项成功。运行选择器在此提交上仍返回 **G1/IMP-048**；下一轮须从最新 master 重算，剩余范围见阶段页。用户临时授权 Codex 免网页审核自动推进，直到要求“改回来”；阶段门、完整门禁与准确 HEAD 发布回执仍适用。
...
**IMP-048 当前开发现场（2026-09-25）**：第十六片 PR #146 准确 HEAD `2de7c90468c1e0f23251b23d270ad7225db5f7d9` 已合并为 `e7f48481c5f0c593c4465b74dbd55b3742e6c9e2`；作者 `DegradedRelease` 回执 `5817590539` 精确绑定该 HEAD，临合并 `release_check.py exit 0`，PR CI `36024090034` 与合并后 master CI `36024606016` 三项均成功。修前复现较新的 301 条无关题材方向遮蔽六小时窗内较早的真实匹配消息；现先按目标题材筛选，再截有界近期窗口，保持方向与解释版本同查询。本机定向 **17 passed**、全后端 **4335 passed / 80 skipped / 0 failed**，前端常规/UTC 各 **717 passed**，全量 pyflakes、tsc、eslint、Next build、文档体检、卫生及公开仓扫描通过。隔离性能样本不代表生产；匹配消息不证明其先于走势启动。本机旧库无目标事件/水位表，未做目标运行库缺口探测、迁移或生产加载，Jev Review 只是作者反馈。代码分支已清理，IMP-048 仍为 `进行中`，剩余范围见 [W03/IMP-048](stages/w03-execution.md#imp-048)。
...
**IMP-048 传播核对**：本片修正 W03 已登记的板块异动消息归因范围，不新增判断事实源、业务目标或发布权限。所属阶段状态与剩余范围已更新 W03，本 handoff 记录最近发布与运行边界；代码契约在 `backend/app/events/store.py`。总方案、领域蓝图、Jev 蓝图、plan-registry、INDEX、AGENTS 和 Skills 均无规则变化，故不适用；没有新增文档或第二任务账本。
