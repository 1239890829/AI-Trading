...
> **定位 / 摘要（2026-09-24 当前）**：PR #114/#115 已合并；IMP-048 三片 PR #116/#117/#118 均已合并，第三片合并点 `b37b01a2d171395a41c41a67f7833dabd798f192` 的 CI `35952611453` 三项通过，三条功能分支已删除。运行选择器继续返回 **G1/IMP-048**；第四片 `codex/imp048-flash-recovery` 正处理超过常规补采 20 页的缺口，尚未发布。用户临时授权 Codex 免网页审核自动推进，直到要求“改回来”；阶段门、完整门禁与准确 HEAD 发布回执仍适用。
...
**IMP-048 当前开发现场（2026-09-24）**：第三片 PR #118 准确 HEAD `ea7367977c7ec253196fe8486002f6ade6eef9fb` 已合并为 `b37b01a2d171395a41c41a67f7833dabd798f192`；本地全后端 **4303 passed / 80 skipped / 0 failed**、定向 **86 passed**，PR CI `35952187356` 与合并后 master CI `35952611453` 的后端/前端/文档三项及必要步骤均成功。`DegradedRelease` 回执 `5807148587` 精确绑定该 HEAD，发布检查返回 `exit 0`；本地/远程功能分支已清理、工作区干净。本机无 8000/3000 监听服务，未声称运行态加载验收。`jev-review` 三片调用均因本机未配置 `JEV_API_KEY` 失败，没有评分。IMP-048 仍为 `进行中`，未完成范围见 [W03/IMP-048](stages/w03-execution.md#imp-048)。

**第四片开工现场（2026-09-24，待发布）**：基于 `master@fa2c069b970a8da57672ded22193f318dd66d2f9` 的运行选择器返回 G1/IMP-048；旧水位超过 20 页时会持续重复拉取却保留 gap，属于有界补采的恢复自锁。上游只读实测连续 21 页、各 50 条且 `sortEnd` 未循环。当前分支新增显式手动深补入口、默认只读预检、SQLite 备份和准确旧水位前置；隔离快讯测试 **28 passed**、本机全后端 **4306 passed / 80 skipped / 0 failed**、前端常规与 UTC 各 **713 passed**，全量 pyflakes、tsc、eslint、Next build、文档体检、卫生及公开仓扫描通过。PR/CI、准确 HEAD 回执均尚待执行。未触碰真实运行数据库，未宣称 100 页外或来源删 ID 的历史完整性；Jev Review 本片已取得 baseline，评分只是辅助自检。
