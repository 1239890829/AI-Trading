...
> **定位 / 摘要（2026-09-26 当前）**：IMP-048 第二十九片 PR #164 已合并为 `77ef6b3fd5a185cc920593d14a22e31e2ca3cd04`，合并后 master CI #36206408019 三项通过；第三十片正在修正次日题材关注聚合的事件可见时点与撤销状态，发布仍以准确 HEAD 回执、CI 与合并后核验为准。阶段页记录本轮反例与剩余范围。用户已要求持续推进 IMP-048 直到任务完成；临时授权 Codex 免网页审核自动执行，直到要求“改回来”，阶段门、完整门禁与准确 HEAD 发布回执仍适用。
...
**IMP-048 当前开发现场（2026-09-26）**：第三十片隔离反例：次日题材关注的 4 条事件中，未来发布、解释尚未可见、已撤销的 3 条原先仍计入净方向。现按同一查询的发布时间、解释可见时间、活跃状态和待复核状态在限额前筛选，旧卡缺版本仍为未知；单测修前判红、修后通过，本机全后端 **4356 passed / 80 skipped / 0 failed**。第二十九片 PR #164 已完成准确 HEAD 发布与 post-merge CI。旧库筛查队列 36 条已全部作者预标：22 条标题直接关联、2 条原文核实非直接关联、12 条未知；其中只有 3 条有原文核验，独立人工 gold 仍为 0。作者预标不冒充独立人工 gold。默认 `data/ashare.db` 仍为旧 schema `b5c9e7a2d4f1`，`backend/data/ashare.db` 是空文件，8000 端口无后端实例；无准确新版运行库验收或生产加载。IMP-048 仍为 `进行中`，剩余范围见 [W03/IMP-048](stages/w03-execution.md#imp-048)。
...
**IMP-048 传播核对**：本片只收紧现有次日题材关注的事件可见性与状态筛选，不新增评分权重、交易动作或发布权限。所属 W03 stage 与本 handoff 记录反例、当前发布和运行边界；代码契约在 `backend/app/events/store.py` 与 `backend/app/api/routes/events.py`。总方案、领域蓝图、Jev 蓝图、plan-registry、INDEX、AGENTS 和 Skills 均无长期规则变化，故不适用；没有新增文档或第二任务账本。
