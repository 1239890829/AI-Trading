...
> **定位 / 摘要（2026-09-26 当前）**：IMP-048 第二十六片 PR #161 已合并为 `52f8b03221a1e6ae728d761fbe7db4c5faba4498`，合并后 master CI #36168740645 三项通过；第二十七片补齐助手资讯工具的解释版本证据，代码提交 `2fc77dd7dff4d4fe1c61a038993ebbc6b1f05d1d`，发布仍以准确 HEAD 回执、CI 与合并后核验为准。阶段页记录本轮修复与剩余范围。用户已要求持续推进 IMP-048 直到任务完成；临时授权 Codex 免网页审核自动执行，直到要求“改回来”，阶段门、完整门禁与准确 HEAD 发布回执仍适用。
...
**IMP-048 当前开发现场（2026-09-26）**：第二十七片修前定向反例：助手 `news` 工具直接输出规则/LLM 方向与依据，却不显示解释版本；旧卡容易被当作有当时版本证据。现与市场事件列表共用 `EventStore.list_events` 的当前解释引用，逐事件显示版本、可见时点和状态；缺版本/可见时点明确未知。30 条事件触发工具 1500 字截断时，原末尾“方向只是推断、不构成买卖建议”消失；现放在列表前，截断仍可见。定向测试通过；最终代码本机全后端 **4349 passed / 80 skipped / 0 failed**，前端常规/UTC 各 **717 passed**，全量 pyflakes、tsc、eslint、Next build、文档体检、卫生和公开仓扫描通过。发布以准确 HEAD 回执、CI 与合并后核验为准；未迁移旧运行库或加载生产实例，IMP-048 仍为 `进行中`，剩余范围见 [W03/IMP-048](stages/w03-execution.md#imp-048)。
...
**IMP-048 传播核对**：本片让已有助手资讯消费者展示 W03 已登记的解释版本与可见时点，不新增权重、判断事实源、业务目标或发布权限。所属阶段状态与剩余范围已更新 W03，本 handoff 记录当前发布与运行边界；代码契约在 `backend/app/assistant/tools/events.py`。总方案、领域蓝图、Jev 蓝图、plan-registry、INDEX、AGENTS 和 Skills 均无长期规则变化，故不适用；没有新增文档或第二任务账本。
