...
- **门禁基线（`RSH-003` 切片轮收尾实测，接手时可直接对照）**：后端 **collect 3306
...
| 任务 ID | 状态 | 日期 | 一句话 |
|---|---|---|---|
| `RSH-003` | 🟡 部分闭环 | 2026-09-16 | 候选因子「入池前结构新颖性筛查」切片交付（三判据 + CLI + 18 例）；**四候选真实库实测**：`willr20`/`cmo20` 论证重复（+1.000）· `kurt20` 提示冗余（0.765）· `skew20` 无冗余证据（0.523）；同轮修掉**薄样本配对劫持择优结论**与**CLI 参数被静默丢弃**两处缺陷 |
...
- **门禁**（2026-09-16 收尾实测；后端跑时 8000 在跑）：后端 **collect 3306 / 3230 passed /
  76 skipped / 0 failed**（**236.05s**）· `pyflakes` **0** · `doc-health` **全部通过** ·
  `tsc` **0** · `eslint` **0 error / 0 warn** · 前端 **608 项 / 67 文件**（本地与 `TZ=UTC`
  **逐字一致**；`apps/web/` 本轮**零改动** ⇒ 与上轮基线相同）。
  ⚠️ **Δ 机械归因（较上值 3287 / 3211 ⇒ +19 collect = +19 passed / ±0 skipped，非覆盖率虚增）**：
  `+18` = 新文件 `tests/test_factor_novelty.py`；`+1` = `test_import_lint.py` 分层参数化新增一个
  **业务层**模块 `app/factors/novelty.py`（实测用例名
  `test_business_layer_never_imports_api[-factors/novelty.py]` 存在）。
...
- **遗留与下一步**：
  ...
  - 若把候选扩到 **ADX / PPO**（`RSH-003` 原文点名的优先项），需按**分批**跑：
    160 列已 **295s**，列数是主导成本项，且成本随截面日数线性增长（可先用 `--lookback` 小窗做初筛）。
