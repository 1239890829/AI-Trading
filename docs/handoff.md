...
- **发布门 companion 已闭环**：PR #64 reviewed HEAD `dd75593e3daf6c2343b41f0e91160c4f7aadc167` 的网页 `Review` 回执为 comment `5749537877`；required CI run `35507688071` 三项全绿（backend `4023 passed, 80 skipped` + pyflakes success）。随后在隔离临时检出中真实运行新版 `release_check.py 64 --expected-head dd75593e...`，脚本读取到同一回执并 exit 0；临时检出已清理。PR #64 合并为 `master@6cbe746750d02bd387ead71d5d85cec50445e8e5`，post-merge push CI run `35508271019` backend/frontend/docs 亦全部 success。现在缺回执、错 PR、旧 HEAD 都会被 release gate 阻断；该证明仍只代表流程一致性，不代表同 GitHub 账号的密码学身份隔离。
- **当前允许动作**：治理 companion 已退出当前候选；主门保持 G1，下一主切片恢复为 **IMP-044**。其 Preflight 已证实必须同时解决：① brief 去重早于 AlertEvent/Outbox 导致部分失败后永久漏提醒；② interactive Feishu 直发绕过 channels/Outbox 形成双事实源；③ bool 回执混同 explicit rejection 与 unknown。旧“聚合卡”注释在同一通知切片内随真实语义收口。IMP-049/053 的动态 actionability 冻结问题继续留原 owner，不塞进 IMP-044。
