...
- **P1-27 selected-only ORM 放大**：即使不再使用巨型 `IN`，旧 selected-only 实现仍先把同日全部 snapshot materialize 到 Python 再过滤；真实 2026-09-18 是 81,672→8。当前 selected-only 分支改为 SQL count + SQL selected predicate，仅 materialize 8 条 actionable snapshot；同一真实副本 no-op horizon 探针从约 1.82s 降至 0.57s。full-funnel 显式离线模式仍允许全量读取，但不会进入生产 EOD。
- **P1-27 selected-only 仍全量 ORM 扫描**：初版虽不再写 deferred future identity，但仍先加载 81,672 条当日 snapshot 再在 Python 筛 8 条；真实副本 no-op 计时约 1.82s。当前改为 SQL 侧 selected predicate + 独立 `COUNT(*)` 保留 source denominator，同行为复测约 0.57s；回归测试锁定 selected-only 模式只加载 selected snapshot 对象。
