...
- **门禁基线（本次收尾实测，接手时可直接对照）**：后端 **collect 3247（3171 passed / 76 skipped / 0 failed）**、
  前端 **598 项 / 66 文件**（本地时区与 `TZ=UTC` 一致）、`eslint` **0/0**、`doc-health` **全部通过**。
  （前端较上轮 593/65 的 **+5 / +1** 即本轮新增的气泡用例文件，非覆盖率口径变化。）
...
- **门禁**：后端本轮**未改动**（纯前端 + 文档），故沿用上轮基线 **collect 3247（3171 passed / 76 skipped / 0 failed）**；
  `doc-health` 见本轮收尾实测。
