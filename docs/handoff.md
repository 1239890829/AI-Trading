...
> **定位 / 摘要（2026-09-26 当前）**：IMP-048 第二十二片在 PR #157；准确发布状态以该 PR 的 HEAD 回执、发布检查和合并后 master CI 为准。阶段页记录本轮修复与剩余范围，下一刀须在最终 master 重算选择器。用户已要求持续推进 IMP-048 直到任务完成；临时授权 Codex 免网页审核自动执行，直到要求“改回来”，阶段门、完整门禁与准确 HEAD 发布回执仍适用。
...
**IMP-048 当前开发现场（2026-09-26）**：第二十二片 PR #157 修正直接详情/标的池的未来证据漏口。隔离 API 先判红：未来发布或未来才生效解释的事件仍在详情显示当前方向，并进入标的池；现尚未可见时方向和判定列为 `unknown`，标的池暂停，原始解释版本保留审计，到达可见时点恢复。作者 U49 核了旧版未知、点时边界、详情/标的池消费者、审计和无新增权限/评分；同一拍时间贯穿暂停判断及序列化。定向 **116 passed**、全后端首次无关计时探针失败后同树重跑 **4343 passed / 80 skipped / 0 failed**，前端常规/UTC 各 **717 passed**，全量 pyflakes、tsc、eslint、Next build、文档体检、卫生及公开仓扫描通过。最终合并证据以 PR #157 的准确 HEAD `DegradedRelease`、发布检查和 master CI 为准；本片未加载生产实例，IMP-048 仍为 `进行中`，剩余范围见 [W03/IMP-048](stages/w03-execution.md#imp-048)。
...
**IMP-048 传播核对**：本片修正 W03 已登记的详情/标的池点时证据语义，不新增判断事实源、业务目标或发布权限。所属阶段状态与剩余范围已更新 W03，本 handoff 记录当前 PR 与运行边界；代码契约在 `backend/app/events/store.py` 与 `backend/app/api/routes/events.py`。总方案、领域蓝图、Jev 蓝图、plan-registry、INDEX、AGENTS 和 Skills 均无长期规则变化，故不适用；没有新增文档或第二任务账本。
