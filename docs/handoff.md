...
**当前模式：READY_FOR_NEXT_PLANNED_SLICE。** v9.9 继承并继续执行 U45 阶段门治理；U47 已把猎场双轨验证、动态盘中重评和 UI/动效治理写入后续阶段，U48 又把所有机制的阶段性有效、衰退重验、知识贡献和成本收益反馈登记为 GX/GOV-027，但两者都不改变当前主门；BUG-029 已由 PR #55 修正“曾封板身份 ≠ 当前封板状态”，并把机会证据版本升至 v2。BUG-020 的真实交易时段生产会话验收仍为 G0 `待条件`；条件未满足时不计当前可行动阻断项，故当前主门仍为 G1，下一主切片按门内序为 **IMP-006**，之后 IMP-044。若 BUG-020 条件变为可行动，低门 G0 必须重新优先。BUG-029 的状态正确性修复不构成策略收益或成交保证。
...
- U48 同样不制造跨门例外：W08/GOV-027 为 `GX/P1/持续治理/门内序47`，只可作为不冲突的伴随切片；它先复用 factor/strategy/KB/Jev/opportunity 各自 owner 的既有证据，定义最小生命周期/衰退/成本反馈契约，不建第二总注册表。任何生产降权、阈值变化、策略/Jev 晋级仍回原 owner task 与证据门。
