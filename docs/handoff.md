...
- **数据源审计切片**：独立分支 `codex/data-source-routing-audit`，基于 `9364ef0`。
  本切片后端 **3360 collect（3284 passed / 76 skipped / 0 failed，248.83s）**；8000在跑，起始load 5.36/6.76/7.16。
  前端本地/UTC均 **608 passed / 67文件**；`tsc`/`eslint`/`pyflakes`/生产构建通过。
  `doc-health`仅 **N/O未通过**，原master干净检出同样复现；本切片**未合并、未部署**。
  下列已合并基线是历史记录；具体证据见 §BUG-018，不能沿用另一工作区数字。

- **历史已合并分支（9364ef0）**：`master`。接手审阅偏差修复已由
  `codex/fix-handoff-review-deviations` 经 PR #19 合并，功能分支在交付后删除。
- **该轮已交付基线**：当时 `origin/master`；交付后
...
## BUG-018 行情身份与指数路径已修，失败语义仍待闭环
- **账本**：`docs/retro-and-gaps.md` §6.0 `BUG-018` ｜ **日期**：2026-09-17 ｜ **状态**：🟡 部分闭环
...
- **注入自证**：本项为评估，不新增运行能力；正确性复现归 `BUG-018`，不重复计算覆盖。
- **门禁**：随 `BUG-018` 的同树完整检查；暂无生产性能/收益提升结论。
