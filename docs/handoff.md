...
> **定位 / 摘要（2026-09-26 当前）**：IMP-048 第二十八片 PR #163 已合并为 `7bfc1c1001af107b80c14355d298b3005349511c`，合并后 master CI #36173370959 三项通过；第二十九片正在修正东财按代码搜索新闻被误当个股权威关联的缺口，发布仍以准确 HEAD 回执、CI 与合并后核验为准。阶段页记录本轮反例与剩余范围。用户已要求持续推进 IMP-048 直到任务完成；临时授权 Codex 免网页审核自动执行，直到要求“改回来”，阶段门、完整门禁与准确 HEAD 发布回执仍适用。
...
**IMP-048 当前开发现场（2026-09-26）**：第二十九片从旧库和来源原文发现，按 `600293` 搜索得到的“歌尔股份否认有玻璃基板业务收入”被误标成三峡新材权威个股关联；原文仅在板块表现段提到三峡新材。旧库同类 2493 条中有 660 条标题不含查询代码或公司名，此为待核候选数量，不是错误率。隔离采集测试修前判红；现只有标题出现代码或已知公司名才建立直接个股关联，其他文章保留原文但标的关系待核；批量补名称失败仍保持未核。隔离 SQLite 定向采集 **5 passed**、最终代码本机全后端 **4355 passed / 80 skipped / 0 failed**，前端常规/UTC 各 **717 passed**，全量 pyflakes、tsc、eslint、Next build、文档体检、卫生和公开仓扫描通过；发布仍以准确 HEAD 回执和 CI 为准。旧库筛查队列 36 条、作者初标 6 条、独立人工 gold 0 条；旧库未被改写且无新版事件/水位表。作者初标不能冒充独立人工 gold 或真实运行库验收。IMP-048 仍为 `进行中`，剩余范围见 [W03/IMP-048](stages/w03-execution.md#imp-048)。
...
**IMP-048 传播核对**：本片只收紧现有新闻采集的个股身份依据，保留原文与题材候选，不新增评分权重、交易动作或发布权限。所属 W03 stage 与本 handoff 记录反例、当前发布和运行边界；代码契约在 `backend/app/api/routes/events.py`。总方案、领域蓝图、Jev 蓝图、plan-registry、INDEX、AGENTS 和 Skills 均无长期规则变化，故不适用；没有新增文档或第二任务账本。
