...
- **当前切片**：PR #29 已合并 `18f0eb4`，准确 PR/master 首次三 job 全绿；回执在 artifacts/runs/ci-offline-20260917/verification/delivery.json。本轮 `codex/notification-outbox` 从该主干开始，实施 `IMP-044` 首片及 `GOV-018` 导入环工具迁移；8000/3000 停用，未发送真实通知、未运行生产库迁移。
- **最新预算**：PR #29 后账户页 1920/2000 已用，剩余 80，预留 40、常规 40，billable $0；该批实耗 18 分钟。本批继续集中交付、记录两轮各 job/安装/测试时间；不为度量额外触发 Actions。前端 threads 对照无收益（71.42→73.11s）不采用；本地因子剖析主要耗时在 SQL，未改样本/口径，后续仍按 IMP-043 验证。
- **本批完整门禁（IMP-044 / BUG-024 / GOV-018）**：后端 **3716 collect / 3638 passed / 78 skipped / 0 failed，96.43s，224 文件**；前端本地/UTC **675/71**，单 worker **40.00s / 38.30s**；tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。8000/3000 停用，前后端全量串行；后端期间有文档、静态检查及独立恢复演练，负载抽样 3.90/3.19/3.55，非空载基准。
  基线 `18f0eb4` 的 **3670 → 3716**：新增 **46** = Outbox/冷却 **37** + 工具 **7** + 两个非业务模块 import-lint 跳过 **2**；原用例身份全保留，skipped **76 → 78** 已对账，前端源码零差异。恢复演练独立进程：pending 退出后重启受理一次，再重启和备份恢复均 0 次发送（仅隔离 HTTP transport）。证据存 artifacts/runs/notification-outbox-20260917/verification；准确 PR/master 另随交付回执核验。
- **上一批完整门禁（IMP-043 / BUG-023 / BUG-012）**：后端 **3670 collect / 3594 passed / 76 skipped / 0 failed，154.87s，222 文件**；前端本地/UTC **675/71**，单 worker **94.41s / 74.31s**；tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。8000/3000 停用，前后端全量串行；最终后端期间静态检查与只读取证，收尾负载 **6.60/6.94/5.91**；前端本地轮期间另跑 tsc/eslint。最终全量期间 app/tests 源码哈希不变。
  基线 `b5a9f7b` 的 3622/220 文件 → 3670/222 文件，**+48** = 回执 **39** + 网络隔离 **8** + 后台议程隔离 **1**；旧用例身份无删改、skipped 不变，前端 675 项身份不变。原冒烟/个股资金流断言按 AST 核对全部保留。证据在 artifacts/runs/ci-offline-20260917/verification；本批推送前合并了多个本地切片，不为每次提速单独跑 Actions。
- **上一批提交前快照（2026-09-17）**：PR #27 已合并 `8a16c3c`，PR/master CI 三 job 均通过；回执在原项目 artifacts/runs/minute-axis-20260917/verification/delivery.json。当前分支 `codex/index-cold-completeness`，工作区 `/tmp/ashare-index-completeness`，实施 BUG-020 冷启动指数完整性及 GOV-018 盘后复盘入口提炼；8000/3000 均未启动，生产未加载本批代码。远端交付以本分支 PR 与归档回执为准。
- **上一批发布前预算**：本批发布前 14:52–14:53 账户页实测 1883/2000 已用、剩余 117 分钟，预留 40、常规 77，billable $0；前一批 135 分钟为历史快照。本批两切片共用一次完整 PR 与 master CI，发布后回执记录实际增量，不改变计费或跳门禁。
- **上一批完整门禁（BUG-020 / GOV-018）**：后端 **3622 collect / 3546 passed / 76 skipped / 0 failed，170.44s，220 文件**；前端本地/UTC **675 passed / 71 文件**，单 worker **65.36s / 65.53s**。8000/3000 停用，前后端全量串行；后端期间文档与只读取证，收尾负载 13.23/11.13/7.37；前端期间有只读核查及短专项测试。tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
...
- **分钟预算（2026-09-17 13:07 账户页面只读刷新）**：1,848/2,000 已用，实际剩余 **152 分钟**，14 天后重置，billable $0；常规 **112**、收尾/故障预留 **40**。PR #25 与 master 合计消耗 17 分钟，估算与账户增量相符；本批按完整验收后一次 PR 交付，不启用付费、不删测试或跳检查。原始数值见本批 artifacts/verification/billing-before.json。

- **上一批完整门禁（BUG-020 东财指数响应身份）**：后端 **3596 collect / 3520 passed / 76 skipped / 0 failed，164.42s**；前端本地/UTC **651 passed / 70 文件**，单 worker **75.75s / 88.18s**。8000/3000 停用、前后端全量串行；后端期间有隔离 ASGI 与旧工作区归档，负载抽样 3.15/3.28/3.34；前端期间有只读核查、文档回填和账单页面操作。tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
  `e976c10` 的 3578/218 文件 → 3596/219 文件，全部 **+18** 为新增东财 HTTP/Hub 回归；旧用例身份无删改，import-lint 269、skipped 76 不变。前端源码零差异，651 用例身份全部相同。OpenAPI **170 paths / 零差异**；实际隔离 HTTP Provider→Hub→ASGI 验收 401、完整/缺失/恢复均 200，逐指数 high→stale→high，原价与时间在缺项时保留；非真实上游或盘中 SLA 证明。已随 PR #26 合并，PR/master CI 均通过。
- **上一批完整门禁（IMP-042 与指数缓存集成）**：后端 **3578 collect / 3502 passed / 76 skipped / 0 failed，141.36s**；前端本地/UTC **651 passed / 70 文件**，单 worker **57.28s / 61.03s**。8000/3000 停用、前后端全量串行；后端期间有只读核查、OpenAPI 导入与 pyflakes，宿主负载抽样 3.41/3.54/3.41。tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
  相对 `a36946e`：3574 → 3578、217 → 218 文件，全部 **+4** 为缓存热态回归，旧用例身份无删改，import-lint 269 与 skipped 76 不变；前端全部 651 用例身份相同。完整 OpenAPI **170 paths / 零差异**；该批已随 PR #25 合并并完成 master CI，收据见 §IMP-042。
- **上一切片完整门禁（BUG-020 指数缓存缺失）**：后端 **3574 collect / 3498 passed / 76 skipped / 0 failed，186.05s**；8000/3000 未运行，前后端全量串行；期间有只读核查、隔离 ASGI 与 OpenAPI 导入，宿主负载抽样 6.76/5.49/5.82。前端本地/UTC **651 passed / 70 文件**，单 worker **101.69s / 67.60s**；tsc **0**、eslint **0/0**、pyflakes **0**、构建通过。
  与 `f6182c6` 全量 JUnit 比较，3566 → 3574 的 **+8** 全在新指数回归文件，旧用例身份无删改；文件 216 → 217（按跟踪模块匹配 JUnit classname，而非把测试类当文件），import-lint 269、skipped 76 均不变。前端 +1 质量显示回归；doc-health **23 条索引 / 0 档位冲突**，完整 OpenAPI **170 paths / 零差异**。实际隔离路由：401；完整/缺失/恢复均 200，指数质量 high → stale → high。证据归档至原项目 artifacts/runs/index-freshness-20260917，本切片尚未推送。
- **上一批 CI 优化门禁**：本地后端 **3566 collect / 3490 passed / 76 skipped / 0 failed，141.26s**（服务停用、前后端串行），前端本地/UTC **650/70**、静态/构建/文档通过。PR #24 云端 pytest **3490 passed / 76 skipped，401.73s**；backend job **443s**，上轮 **600s**（两次 runner/负载不同，不当作受控同机实验）；三项 job 均成功。按 job 墙钟逐项上取整估算本次 PR 约 **12 分钟**，不是账户账单再次实测；master CI 另计 **9 分钟**，本批合计估算 **21 分钟**；以先前 190 分钟余额扣减，预计剩 **169**（常规 **129**、预留 **40**），上述估算已由本次账户页 1,831/2,000 实测核对；后续活动仍须刷新。
...
- **在途交付**：[PR #24](https://github.com/1239890829/AI-Trading/pull/24) 已合并为 `9319e65ecbd511e5103380a1c0afe1e78b682dcf`；准确 HEAD `f6182c65a72ace20feb50310266affdb5c163e6d` 的 run `35178985075` attempt 1 三 job 全成功、release-check 两次快照一致。master run `35179587782` attempt 1 三 job 全成功；主干实际树与已验收 HEAD 一致，本机 master 已快进，服务保持停用。旧工作区长证据归档后清理，恢复包与未知本机资产保留。
...
| 任务 ID | 状态 | 日期 | 一句话 |
|---|---|---|---|
| `BUG-022` | 🟡 实施中 | 2026-09-17 | 分时图坐标一致性与增量刷新 |
| `IMP-042` | ✅ 闭环 | 2026-09-17 | CI 测试环境与源码解析提速 |
| `IMP-043` | 🟡 实施中 | 2026-09-17 | 持续 CI 成本治理与离线边界 |
| `BUG-023` | ✅ 闭环 | 2026-09-17 | 飞书受理回执真实性 |
| `BUG-024` | 🟡 实施中 | 2026-09-17 | 规则冷却时区 |
| `IMP-044` | 🟡 实施中 | 2026-09-17 | 规则告警飞书 Outbox 首片 |
...
## BUG-022 分时图坐标一致性与增量刷新
- **账本**：`docs/retro-and-gaps.md` §6.0 `BUG-022` ｜ **日期**：2026-09-17 ｜ **状态**：🟡 实施中
- **缺口与验收**：最终融合方案 W01 / §3.5；单边越界导致不对称价格轴与对称百分比轴错位，增量点不刷新轴域；要求真实 Canvas 与纯函数、组件同时验证。
- **改动**：统一价格域及叠加线范围，无效价格留空并提示，实际限价透传，跨日重建槽位、切股清理。价格轴保持自动范围并禁用独立纵轴拖动，时间缩放保留；未扩充后端时间/限价状态契约。
- **机械证据**：轴函数 22 + 组件 12 = **34 passed**；实际 lightweight-charts Canvas 旧实现 **0/8**、修复后 **46/46**，覆盖单边越界、叠加、增量、跨日、缩放、明暗主题及 960/360 宽度，最大坐标误差约 **1.48e-12px**。纵轴拖动曾复现约 44.49px 错位，修后相同操作对齐。全部为明确标记的隔离夹具，原用户图例不据此宣称复现。
- **注入自证**：最终测试回退两份旧生产实现 ⇒ **24 failed / 10 passed**，还原后 **34 passed**；两份 SHA-256 前后相同，收据 artifacts/verification/injection-receipt.json。最早 31 项初测已由本轮 34 项覆盖。
- **门禁**：完整本地数字与负载见 §1；PR/master CI 按 §6.5 核验，远端结果附交付回执；本项保持开放。
- **遗留与下一步**：用户原始图例缺具体标的/日期/载荷，保持未复现；后端参考日/交易日/四态限价契约及其它 P0 仍按主方案推进。
...
- **门禁**：完整本地验收见 §1；无生产库写入或服务启动，HTTP 响应由隔离夹具提供；PR #24 的准确 HEAD 与 master CI 均已通过，交付身份见 §1；仍未部署生产。
...
- **遗留与下一步**：当时下一切片为题材写保护，现已随 PR #24 交付；本 ID 仍开放指数完整性、时间戳质量、生产实例及多窗口复验。
...
- **指数缓存缺失切片（2026-09-17）**：旧实现仅更新返回的指数，漏项仍保留 high；新增 8 条行为回归在旧实现上 7 failed / 1 passed，修复后指数/自选/新鲜度相关 39 项通过。现在缺失的旧指数保留价格、源时间和接收时间，并在 REST/订阅广播前标 `stale(index_batch_missing)`；缺失集变化才记日志，恢复与确定休市沿用现有规则。前端质量徽标解释「本轮未返回该指数，展示旧行情」。
- **该切片边界**：只核验已缓存的指数，不把缓存集合当上游请求全集；首次缺失保持缺席、不造值。东财空字段占位、首次请求完整性、源时间真实性、忽略质量的研究消费者与生产多窗口验收仍归本项，不能据此次缓存修复销账。完整本地门禁已回填 §1；回退旧生产实现后再次 7 failed / 1 passed，还原 SHA-256 一致，8 项复绿；该缓存切片已随 PR #25 合并，master CI 收据见 §1。

- **东财指数响应身份切片（2026-09-17）**：旧实现按裸六位代码关联并为漏项生成空报价，覆盖 Hub 的旧价格且掩盖缺失。新实现只接收请求内完整市场/代码身份，返回原顺序真实行；不制造缺项，重复身份或行结构错误抛 ProviderError。18 条独立 HTTP/Hub 回归在旧实现上 15 failed / 3 passed，修后连同关联回归 73 passed；再次回退旧实现 15 failed / 3 passed、恢复 SHA-256 相同，完整本地门禁与隔离 ASGI 验收见 §1。已随 PR #26 合并 `f9870d3`，PR/master CI 三 job 均通过；运行验收仍为隔离上游夹具。
...
- **GitHub secret protection**：仓库已启用 **Secret Scanning** 与 **Push Protection**，当前 open alerts=0；同时启用 Dependabot security updates 与 **Private Vulnerability Reporting**（外部安全问题可私下报告，避免在公开 issue 泄漏细节）。尝试启用 non-provider patterns / validity checks 后平台仍返回 disabled，因此不能把这两项宣称为已启用，当前自定义扫描器负责补常见 non-provider / 项目特有模式。
- **Dependabot / npm 依赖安全修复（2026-09-19）**：公开仓库初始有 4 个 open Dependabot alerts：归档 GSAP React 示例的 Vite **1 high + 2 medium**，以及 `hithink-finance-cli` 的 Vitest **1 medium**，全部属于 development dependency，不是交易生产运行依赖。GSAP 做最小兼容升级：`@vitejs/plugin-react 4.3.3 → 4.4.0`（4.4.0 是声明支持 Vite 6 的最小版本）+ `vite 5.4.10 → 6.4.3`，两者固定 exact；从空 `node_modules` 重装后 Vite 6.4.3 production build 成功，另用一次性临时 lock + 官方 npm registry `npm audit` 验证 **0 vulnerabilities**，一次性 lock 不提交进归档示例。Hithink 用 exact `vitest 4.1.10 → 4.1.11` 并同步 `npm-shrinkwrap.json` 的 Vitest 族补丁条目；官方 npm audit 进一步暴露 3 个**传递 high**（`brace-expansion 5.0.7` / `nanoid 3.3.15` / `postcss 8.5.16`），因此保留经 audit 证明必要的安全升级到 `5.0.12 / 3.3.19 / 8.5.28`，而不是为了 diff 最小把漏洞版本恢复。最终 shrinkwrap 经 `npm ci --ignore-scripts` 前后保持一致，`npm run verify` **54 files / 166 tests 全绿**。本机默认 npmmirror 不实现 audit API；另一次官方 npm registry audit 曾因本地旧 metavulnerability cache 把已安装/锁定的 `brace-expansion 5.0.12` 错报为旧漏洞范围，使用**全新临时 npm cache**复核后为 **0 vulnerabilities**。因此安全复核固定为官方 registry，遇到“锁定/实际版本已修复但 audit 仍命中”的矛盾时必须用新 cache 复核；禁止 `npm audit fix --force`。
...
- **导入环工具批（2026-09-17）**：全文审阅本机旧工具 271 行并核对初始恢复包，提炼 SCC/路径算法至 `scripts/audit/import_cycles.py`。修正类体误判延迟、from 子模块漏边及语法失败假绿；限定项目源码、不遍历 venv/软链，两个装配层消费者已切换。7 项回归与旧新对照通过，静态候选不冒充运行时证明；迁移矩阵和限制见 `docs/platform-directory-migration.md` §6。旧原件仍在，待本批主干验收后核哈希、可恢复回收及隔离恢复；未知资产不动。
...
- **恢复与边界**：PR #28 主干三 job 全绿后，原件按已核哈希送入项目可恢复回收站；隔离恢复副本逐字节一致，旧文件及已核为空的父目录退出，doc-health 通过且未重建。回执在 artifacts/runs/index-completeness-20260917/verification/migration-retirement.json；相邻未知资产、Serenity 嵌套 Git 和未审完的 UZI 包保留。未实际运行复盘、发送通知或更改调度，MIG-3/5 整体仍开放。
...
## BUG-012 共享 API 夹具的后台议程隔离
- **账本**：`docs/retro-and-gaps.md` §6.0 `BUG-012` ｜ **日期**：2026-09-17 ｜ **状态**：🟡 部分闭环
- **缺口与验收标准**：旧轮认为本地不可复现、怀疑连接失效；本轮盘后全量出现事件两项和风控一项红，日志明确生成后台议程。自治开关仅停止执行，建议议程仍运行，原 conftest「调度全关」不实。
- **改动**：共享 API client 生命周期内，将议程任务替换为等待停机信号；保留注册/启动/收割，专门调度测试直接调用真实循环。生产行为、默认测试库及所有原断言不变。
- **机械证据 / 注入自证**：固定 2099 年盘后时钟及隔离议程 spy，旧夹具 **1 failed + 1 teardown error**，修后事件/风控/真实议程/调度/冒烟关联 **215 passed / 7.50s**。独立双线程事务探针：StaticPool 读到未提交行、读会话退出后写行丢失；文件库对照不读脏行且写入保留。后者证明一种可行机制，不替代三次失败的完整线程因果记录。
- **门禁**：最终全量见 §1；曾红的 3590 passed / 76 skipped / 3 failed 日志保留于 artifacts/runs/ci-offline-20260917/verification/backend-release.log，不能拿此前全绿覆盖。
- **遗留与下一步**：其它无开关后台任务与共享连接仍需逐项取证，故整体开放；未擅改生产库或删除 refresh。旧轮完整证据留在 Git 历史与本批私有回执。

## BUG-024 规则冷却时间按落库时区解释
- **账本**：`docs/retro-and-gaps.md` §6.0 `BUG-024` ｜ **日期**：2026-09-17 ｜ **状态**：🟡 实施中
- **缺口与验收标准**：alert_rule.last_triggered_at 已由既有迁移统一北京 naive，冷却代码却贴 UTC，300 秒实际约需 8 小时 300 秒；须准确判断 299/300/301 秒边界。
- **改动**：AlertEngine 对 naive 值使用已有 BJ_TZ；aware 值维持绝对时间比较，无数据重写、无冷却参数调整。
- **机械证据 / 注入自证**：同一 SQLite 写入再读取，固定绝对时钟，旧实现两条到期边界失败；修复后 4 项通过，包括 aware 输入。另发现并修正本批 Outbox 新鲜度与 QuoteHub 的轮询窗口不一致，旧实现对应 1 红。
- **门禁**：UTC 关联五文件 73 passed / 3.62s；完整结果见 §1，准确 PR/master 尚待本批交付。
- **遗留与下一步**：本项待远端验收后销账；其它通知入口仍归 IMP-044。

## IMP-044 规则告警飞书 Outbox 首片
- **账本**：`docs/retro-and-gaps.md` §6.0 `IMP-044` ｜ **日期**：2026-09-17 ｜ **状态**：🟡 实施中
- **缺口与验收**：旧主干实测事件 commit 后中断发送，重启又被冷却挡住，无可恢复意图。首片仅接规则告警飞书；事务回滚、并发领取、发送前/后中断、受理后 DB 失败、过期与偏好变化均须有可查终态。
- **改动**：新增 Outbox/attempt 与增量迁移；AlertRepository 同事务写入，AlertEngine 独立领取步骤复用既有循环。规则/目标哈希、时段、范围、现行条件及 Quote 新鲜度在发送前复核；意图有效期与新鲜度沿用 QuoteHub 的 max(poll_interval_seconds, stale_after_seconds)。开始发送前写标记；不确定结果与已开始发送的失联租约记 unknown、不重发，未开始且仍有效的租约才可重新领取。
- **机械证据**：独立 SQLite 连接竞争只能领取一次；两个投递器只发一次；真实 MockTransport 经飞书返回受理或空回执，接受状态不生成 delivered/read。事件 API 增加 channel_states，删除规则保留尝试与冻结内容，避免旧结果污染复用事件 ID。空队列只读，不抢 SQLite 写锁。
- **注入自证**：去掉意图落库 1 红、盲目回收已开始租约 1 红、空回执改成功 4 红；各处还原哈希一致。旧主干缺口与全部长证据存本批 artifacts。
- **门禁/恢复**：完整结果见 §1。迁移只增两表，不回填旧事件、不发送历史通知；隔离旧版本库 upgrade、模型/schema 比对及幂等均验收。回滚应用代码保留 Outbox 证据；schema downgrade 明确拒绝直接丢弃记录，生产迁移未执行。
- **遗留与下一步**：买点卡、watcher/日报/持仓等入口仍走原链；bool 通道无法细分明确拒绝与未知，当前保守归 unknown，类型化回执、渠道消息 ID、有界安全重试、全链路 UI 与时延分母另续本项。没有可信送达/已读回执，不宣称已送达；不存在 exactly-once 外部保证。

## BUG-023 飞书受理回执真实性
- **账本**：`docs/retro-and-gaps.md` §6.0 `BUG-023` ｜ **日期**：2026-09-17 ｜ **状态**：✅ 闭环
- **缺口与验收标准**：空 JSON、空错误码被飞书通道认作成功，可能写入成功通道。须明确整数零码才确认受理；缺失/畸形/冲突回执返回未确认，不自动重试，不当作送达或已读。
- **改动**：`notifiers/feishu.py` 只在至少一个已支持的状态键存在且全部为整数 0 时确认受理；拒绝空/畸形/冲突回执及 Python `False == 0` 误判。日志改称未确认，原成功通道和令牌缓存流程复用；未新增自动重试。
- **机械证据 / 注入自证**：39 条新增用例在旧实现 **24 failed / 15 passed**；修后关联三文件 **85 passed / 1.81s**。覆盖 webhook/app × 文本/事件卡/直接卡六入口、registry 不记成功、非法 token 回执不发消息及每次仅一次发送；全部是隔离 HTTP transport，无真实通知。
- **门禁**：PR #29 源提交 `f62d025`、合并 `18f0eb4`；PR run `35197642868` / master run `35198432202` 均 attempt 1 三 job 全成功。两轮后端均 3594 passed / 76 skipped、前端 675/71；交付回执已核，不重复实施。
- **遗留与下一步**：本片不建立 Outbox，unknown 与拒绝的类型化回执、持久化意图及补偿另归账本 `IMP-044`；不修改渠道偏好或发送真实消息。

## IMP-043 持续 CI 成本治理与离线边界
- **账本**：`docs/retro-and-gaps.md` §6.0 `IMP-043` ｜ **日期**：2026-09-17 ｜ **状态**：🟡 实施中
- **缺口与验收标准**：PR #28/master 慢项表显示四条因子全评约 67s，资金流及事件冒烟存在多秒等待；先核离线隔离，保留原端点/断言，比较受控耗时与云端实际分钟。
- **改动**：冒烟保留真实 app/lifespan/路由/服务和原断言，在 HTTP/TDX 外部 I/O 边界返回显式失败；漏出的 DNS/TCP 尝试在发生前拦截并令门禁失败。资金流/板块测试等待只替换局部模块引用、保留让出事件循环；个股失败用例增加三次 1s 等待参数核验。交接技能加入每批成本复核。
- **机械证据**：同机、生产服务停用、socket 拦截下，原 141 用例身份逐项相同，耗时 **20.73→2.97s**；修前 **42** 次网络尝试，修后 **0**。三文件专项 **157 passed / 3.05s**。长证据在 artifacts/runs/ci-offline-20260917/verification；此值是局部对照，不代表整场 CI 比例。
- **注入自证**：撤销异步 HTTP 边界、恢复真实等待参数分别判红，源码恢复 SHA-256 一致。新增隔离自检覆盖同步/异步 HTTP、requests、DNS/TCP、TDX 和局部等待；原冒烟端点与断言保留。
- **门禁**：本地最终全量与增量归因见 §1；精确远端 PR/master 回执在本批 artifacts，未发布前不冒称云端成功。
- **本批复核**：通知 Outbox、冷却修复与工具迁移集中一次交付；新发现的三条红在本地拦住后再提交，不为试验触发 Actions。本批本地慢项仍以因子全评为首（四条合计 23.32s），与上轮不同负载不直接换算云端收益；准确 PR/master 各 job、安装/pytest 时间与账单增量随回执记录。
- **遗留与下一步**：持续逐批复核成本；因子全评仍保留，未以缩小样本或跳过检查提速。下一片接续 IMP-044 其余通知入口与类型化回执，先做本地独立实验再决定优化。

## IMP-041 后端测试夹具装载与 CI 分钟治理
- **账本**：`docs/retro-and-gaps.md` §6.0 `IMP-041` ｜ **日期**：2026-09-17 ｜ **状态**：✅ 闭环
...
- **机械证据**：9 类实际夹具逐表结构/行数/排序后全部值哈希相同；完整测试 AST 除 15 处装载及导入外相同，88 项相关测试全通过。主夹具两表各 8,100 行，交错对照预热后批量 0.070–0.108s、旧方式 3.514–4.308s；首次批量 1.047s，不能忽略初始化。PR #24 后端 job 600s → 443s，合并后同一树 master job 266s；两轮云端均 3490 passed / 76 skipped（runner 与负载不同，只报告实测、不承诺固定降幅），证据已归档 artifacts/runs/ci-budget-20260917。
...
- **遗留与下一步**：本批按准确 HEAD 的 PR CI 与 master CI 闭环；继续按完整批次核算分钟，40 分钟保留收尾/故障，未保证剩余额度必然覆盖全部未知返工。完整融合方案仍按原账本开放项推进。

## IMP-042 CI 测试环境与源码解析提速
- **账本**：`docs/retro-and-gaps.md` §6.0 `IMP-042` ｜ **日期**：2026-09-17 ｜ **状态**：✅ 闭环
- **基线与范围**：最新 master `9319e65`，独立 `codex/ci-validation-efficiency`；复用已验收的指数缓存提交 `a36946e`，集中交付。9 份纯函数测试改为 Node 环境，DOM 测试保持 jsdom；两组只读源码守卫按完整内容与文件名复用解析，目录扫描与断言仍每次执行。
- **验收**：旧用例身份与断言不减；同路径同大小同 mtime 修改、语法错误、热缓存注入与新增文件仍可发现；完整后端、前端本地/UTC、静态、构建、文档、干净检出与精确 HEAD CI。
- **同机交错对照**：两组源码守卫每轮 202 passed / 74 skipped；未缓存 6.55/6.28s、缓存 4.19/4.15s（含 pytest 启动），缓存 1962 hit / 304 miss，峰值 RSS 约 121–123 MiB → 230–231 MiB，容量上限 512 个解析结果。9 份前端共 96 用例，jsdom 5.42/4.63s、Node 1.62/1.62s；均单 worker、服务停用、前后端对照串行。不是整条 CI 固定降幅。
- **等价与注入**：前端除环境头外逐字节相同；两份后端守卫除导入/解析调用外完整 AST 相同。热缓存注入同路径旧结果缺陷后新增 4 例全部失败，恢复源码哈希一致；修复后 206 passed / 74 skipped。目录扫描、豁免与判定结果均不缓存，现有 AST 消费方只读。
...
- **二次价值审计（2026-09-19）**：改以 TypeSafe 官方 Use Case Map 的 decision shape 为上位准绳，主动删除“装了就用”的机械规则。旧全局 `jev-route` 因不能真正切宿主模型却给每个非简单任务固定增加一次 Jev 已停用并删除活动命令；替换为 privacy-safe `jev-capability-route`，只有 ≥2 个真实可用能力且选择不明显时才调用。BillionsBobby/JevRouter 固定本机 commit `c83660f8370f52124055b771a38d4a7ea06e8434`，typecheck + **44/44** tests 通过；真实 TypeSafe 权限 smoke 中 Jev 以 0.99 选 `edit_code`，Router 因 caller 缺 `write_repo` 正确过滤并安全回退；wrapper 还机械校验 pinned HEAD + tracked worktree clean，版本漂移在网络前拒绝。上游默认完整 request/context receipt 不启用，本机 wrapper 只留 metadata。OpenRouter 当前**不配置**：Jev 经 OpenRouter 仍收费，直接 TypeSafe 已可用；free router 属动态生成模型池，不作为交易/研究生产依赖。本次仓库提交只改治理/参考文档，因此最终门禁刻意不再调用 `jev-review`/`jev-pref`，避免用 Jev 审“是否应该少用 Jev”的文档而制造无效额度；组件价值已经由真实 route smoke 与确定性测试验证。
- **遗留与下一步**：adapter/telemetry 与跨重启 metadata-only usage 历史已完成；`data/jev/usage.jsonl` 明确 gitignore，`backend/scripts/jev_usage_report.py` 可零网络汇总。`GET /api/system/providers` 的 `jev` 字段仍刻意只报进程内即时统计。任何 `shadow → cascade` 的生产阈值必须来自 `RSH-030` 的人工金标准与目标域验证；社区 Jev 组件若真实 30–50 次调用后不改变下一步/不省高级模型 token/升级率过高形成双成本，则进入停用候选。
...
- **缺口与验收标准**：AI 助手的 bounded **tool-group routing** 已接 shadow；代码先声明 6 个只读工具组，Jev 多 Noul 可同时选多组，执行授权仍由 `TOOL_SPECS` 控制。浏览器研究已复用用户级 `jev-browser`。二次审计后**自动 model/effort router 明确暂缓而非主要开放项**：当前宿主不能因一次 Jev Choice 就可靠切换模型，额外代理层也未证明净收益。现阶段开放项收敛为业务 assistant route 的长期 coverage/token 校准；全局只保留多真实能力竞争时的 capability router。
...
- **全局 capability routing 收敛**：不再对每个非简单任务先做“该用哪个模型”的 Jev 分类；这种 routing 不能真正切换宿主模型且会固定多烧一轮。现在只在 ≥2 个真实 Tool/Skill/MCP/CLI/Subagent 候选竞争且选择不明显时使用 `jev-capability-route`。JevRouter 中 Jev 负责概率，代码负责 availability / actor permissions / risk / confirmation / input schema；decision-only，不执行能力。真正的 model/effort 自动切换仍暂停，须另有真实会话 A/B 证明价值。
...
- **独立 Jev 预标注（不是 gold）**：新增 `predict-jev / prioritize / compare-reference`。固定 `jev-1.13.0` 对 240 条完整运行：**40 requests / 206,482 input tokens / 29,441 output / 平均 866.07ms/request**；按 $0.042/MTok input 估算约 **$0.008672**。prediction SHA-256=`c344ce0a881db27f6c22001a43d4d9e3876cd699ddd4be333d3105606a0ddc08`；review-priority SHA-256=`7b5242d941215bcca5365cb7e7e51099fabee31eaa907a5fe351abed143778be`。模型产物绝不写 `human`；CLI 用临时文件+原子替换，首轮真实 API 因 Choice 展示概率舍入触发 fail-closed 时确认**没有半份 predictions/meta**。后续 `prioritize/compare-reference` 强制核对 prediction meta 中的 `queue_sha256` 与 row count；review-priority 另有 sidecar 记录 queue/prediction/review 三个指纹。
- **规则/Jev 分歧只作审核导航，不是 accuracy**：category agreement **133/240=55.42%**、certainty **187/240=77.92%**、actionable **138/240=57.50%**；category 分歧 107、certainty 53、actionable 102。category confidence<0.75 有 90 条、certainty confidence<0.75 有 56 条、actionable Noul 0.25–0.75 有 **150 条**。review-priority 把这些分歧/不确定样本排前，帮助人工先看信息量最大部分。
- **对 cascade 节省假设的削弱证据**：这 240 条平衡研究队列中 actionable Noul **<=0.05 = 0**、<=0.10=9、<=0.20=41、>=0.90=0。故当前 event cascade 的 `<=0.05` 阈值在此队列上会节省 **0/240**；但该队列不是生产 pending pool，因此只能说明“尚无节省证据”，不能据此直接改阈值。必须等 human gold 后做 precision/recall/成本联合校准。
...
- **Gold score 完成度门**：`score` 默认要求全部 human label 完整，0/240 或部分标注时直接拒绝；只有显式 `--allow-partial` 才允许查看进度，并在输出中标记 `partial=true`，不得作为生产阈值依据。prediction event_id 重复也直接 fail-closed；`prioritize/compare-reference` 还会强制核对 prediction meta 的 queue hash 与 row count。
