...
> **定位 / 摘要（2026-09-26 当前）**：IMP-048 第二十七片 PR #162 已合并为 `98ffcc255382dc8ff5086980efee5ca9c901b827`，合并后 master CI #36170853123 三项通过；第二十八片补齐解释回放的来源发布时间门，代码提交 `3c0da121d7adb7a81e38e9f55cd8b99dcc689707`，发布仍以准确 HEAD 回执、CI 与合并后核验为准。阶段页记录本轮修复与剩余范围。用户已要求持续推进 IMP-048 直到任务完成；临时授权 Codex 免网页审核自动执行，直到要求“改回来”，阶段门、完整门禁与准确 HEAD 发布回执仍适用。
...
**IMP-048 当前开发现场（2026-09-26）**：第二十八片隔离反例先判红：来源 11:00 才发布的事件，解释版本 10:00 已写入，10:30 的 `interpretation_at` 仍返回版本。现同一 SQL 要求来源已发布且版本已生效，保留旧版本无历史倒填；定向事件/核验 **62 passed**、本机全后端 **4350 passed / 80 skipped / 0 failed**，前端常规/UTC 各 **717 passed**，全量 pyflakes、tsc、eslint、Next build、文档体检、卫生和公开仓扫描通过。另以本机旧库的隔离 SQLite 副本从 Alembic `b5c9e7a2d4f1` 升至 `a4e8c2d9f6b1`，重复升级和 `quick_check` 通过；原 **11101** 张事件卡仍可读，抽样旧卡版本保持 `unknown`，没有合成历史观察/解释。原库未写入，临时副本已删除。该旧库缺新版表且含晚于当前日期的事件时点，不能用作真实水位或修订关系验收；准确新版库与独立人工标签仍待提供。发布以准确 HEAD 回执、CI 与合并后核验为准，IMP-048 仍为 `进行中`，剩余范围见 [W03/IMP-048](stages/w03-execution.md#imp-048)。
...
**IMP-048 传播核对**：本片修正 W03 已登记的当时可见解释回放，不新增权重、判断事实源、业务目标或发布权限。所属阶段状态与剩余范围已更新 W03，本 handoff 记录当前发布与运行边界；代码契约在 `backend/app/events/store.py`。总方案、领域蓝图、Jev 蓝图、plan-registry、INDEX、AGENTS 和 Skills 均无长期规则变化，故不适用；没有新增文档或第二任务账本。
