...
> **定位 / 摘要（2026-09-25 当前）**：IMP-048 第二十一片在 PR #156；准确发布状态以该 PR 的 HEAD 回执、发布检查和合并后 master CI 为准。阶段页记录本轮修复与剩余范围，下一刀须在最终 master 重算选择器。用户临时授权 Codex 免网页审核自动推进，直到要求“改回来”；阶段门、完整门禁与准确 HEAD 发布回执仍适用。
...
**IMP-048 当前开发现场（2026-09-25）**：第二十一片 PR #156 修正共用活跃事件查询的未来证据。隔离库先复现：10:00 查询包含 10:01 才发布的卡片和 10:01 才生效的最新解释；现在 SQL 限额前排除这两类证据，`is_active` 同步拒绝未来发布时间。作者 U49 检查了版本可见时点、旧版未知、查询限额、直接消费者及审计读取：无版本旧卡保持 `unknown`，非活跃审计查询仍可见原记录，评分、排序和通知未变。定向 **115 passed**、全后端 **4342 passed / 80 skipped / 0 failed**，前端常规/UTC 各 **717 passed**，全量 pyflakes、tsc、eslint、Next build、文档体检、卫生及公开仓扫描通过。最终合并证据以 PR #156 的准确 HEAD `DegradedRelease`、发布检查和 master CI 为准；本片未加载生产实例，IMP-048 仍为 `进行中`，剩余范围见 [W03/IMP-048](stages/w03-execution.md#imp-048)。
...
**IMP-048 传播核对**：本片修正 W03 已登记的活跃事件证据可见时点，不新增判断事实源、业务目标或发布权限。所属阶段状态与剩余范围已更新 W03，本 handoff 记录当前 PR 与运行边界；代码契约在 `backend/app/events/store.py`。总方案、领域蓝图、Jev 蓝图、plan-registry、INDEX、AGENTS 和 Skills 均无长期规则变化，故不适用；没有新增文档或第二任务账本。
