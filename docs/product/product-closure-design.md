...
| 猎场 | 机会/跟踪/影子/复盘职责分区；为什么、还等什么、何时失效；reference 与实际 shadow fill 身份清楚；同股统一详情 | 生成组合/简报、单拍/实验按钮、权重与分类调参 |
...
沿用zinc灰阶、红涨绿跌、tabular-nums、亮暗、紧凑字体与既有CardShell/CardHead/详情导航。主卡只展示改变用户判断的内容，依据按需展开；未知、陈旧、失效及重要风险必须显式可见，不能以简洁为名隐藏。页面 shell 保持 viewport 锁定，业务纵向滚动只发生在明确的内部容器；上下文型详情优先 Drawer/inline，轻量筛选优先 Popover，阻断确认或大画布高聚焦任务才用 Modal，不按组件习惯机械统一。
