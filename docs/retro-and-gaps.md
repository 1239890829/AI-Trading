...
| 35 | 批次 4 死代码 spot-check 结论（2026-09-13） | 「`_perf_middleware` 全仓名字零引用 ⇒ 真死代码，可删」 | **它是被 `@app.middleware("http")`（`main.py:617`）注册的活跃中间件**，是 perf 指标管线（`/api/system/metrics`）的数据源——**装饰器行不含被装饰函数名**，「按名字数引用」的扫描器和人工 spot-check 都对它不可见。⚠️ **第十二类偏差：框架注册型引用对名字计数天然不可见**（路由端点、middleware、fixture、pydantic 校验器同族——上轮扫描排除了 `@router.*` 却漏了 middleware）。**教训：死代码判定必须读定义处上下文（装饰器在名字上方一行），不能只看引用面**；判据已固化进 `scripts/deadcode_scan.py`（装饰器排除 + pytest 约定排除 + 13 项自证）。若照原结论执行，会静默拆掉性能指标管线 |
