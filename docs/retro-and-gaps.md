...
| P2-33 | 量能评分口径的倒 U 改写（**触发式**，[[KB-STOCK-32]]） | 实证（10y 全 A）：极端放量段（量比 ≥1.5 的 D8-D10）隔日收益转弱/转负，`score_capital`「量比 ≥1.5 +12」与 `BREAKOUT_VOLUME_RATIO=2.0` 的有效区只在适度放量段。**触发条件**：①用户拍板是否把资金维量比加分改为倒 U / 极端惩罚（**评分口径变更**）；②盘中触发器口径的专项验证（用 strategy_verify + 分钟数据，与隔日口径分开判）。附带：tech_score 五维同源（trend/macd/kdj/rsi/pattern 同一价格序列）的**截面分数相关分析**（需评分截面数据积累） |
