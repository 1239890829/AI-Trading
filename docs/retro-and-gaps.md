...
| P1-42 | 事件因子评估通道（事件研究法） | ⏳ **第 0 步已完成（2026-09-13）：触发样本量已计量**（`backend/scripts/measure_event_samples.py`，只读可复跑）。实测：方向行 3256，**带方向触发 670**（theme 399 / symbol 199 / market 72，distinct 事件 436）；事件采集 08-31 起步、近 7 日爆发积累 610 条（≈51.5 条/日）；**前瞻窗口闭合样本仅 37 < 120** ⇒ **维持「等时间」，解锁在即**（按当前速率外推约 09-15 前后达标，节假日更晚）。达标后路径不变：先只做评估器、不碰消费侧；市场中性口径复用 `strategy_verify`；⚠️ walk-forward 需处理同日同标的重复触发（实测 104 组，非独立样本）。评估器何时建 = 解锁后另行拍板 |
...
| P2-33 | 量能评分口径的倒 U 改写（[[KB-STOCK-32]]） | ✅ **三项全部落地（2026-09-13 §6.20，用户授权「不需要确认」）**：①`score_capital` 量比子评分改写为倒 U（阈值取 5 日基线量比分位 p90=1.65/p95=2.08：≥2.1 −10 / 1.65-2.1 中性 / 0.9-1.65 +12 / 0.5-0.9 +5 / <0.5 −8；原「≥1.5 +12」整体落在转弱/负区）；②突破确认条件日线近似验证（`verify_breakout_volume.py`）：**「量比 ≥2 确认」是反选**（可成交口径放量突破 +0.079% < 缩量突破 +0.224%，v5≥3 段转负 −0.072%）——**常量暂不改**（盘中触发器口径不同），盘中分钟级专项验证为**剩余触发式残余**；③五维同源假设**证伪**（3 截面无维度对 \|r\|≥0.6，kdj 与趋势类负相关）⇒ 评分卡无需去冗余手术。详见 KB-STOCK-32 附记 |
...
| 3 | **定时 automation 去留** | ✅ **已裁定（2026-09-13）**：**3 条循环保留**（周一 09:20 仓库发现 / 周日 10:00 仓库跟踪周报 / 工作日 15:45 进化总结——后者是每日唯一飞书推送；低频、有经验闭环兜底价值，无新证据支持停掉）。**1 条过期一次性应删**（P2-21 冷启动实测验收，`scheduledAt=2026-09-11T12:05` 已过但仍 ACTIVE）——⚠️ **平台侧动作未完成**：本会话工具面（CronList/CronDelete）查无此实体（ID `58209806`，工作区隔离，实体在 WorkBuddy 平台侧），已实测删除返回 not-found；**下次任一 WorkBuddy 会话用 automation_update 删除该 ID**（或在平台 UI 删），账本在此留痕为唯一提醒位 |
