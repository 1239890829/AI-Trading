...
> 当前实施与权限修订见 [实施校准 v9.2](../implementation-plan.md)，状态唯一归账本 §6.0；本页历史“自动转正/事后回滚”描述不构成新的线上参数授权，保留背景并由 IMP-046 核当前消费者。
...
已有手工配置和拥有者感知的回滚保留，这不是研究效果已证明的声明。独立审阅来源、候选/基线/证据绑定、撤销/到期及批准消费的原子生效仍归IMP-046后续；不新建未验证的自动批准平台。A类入影子不回写复盘applied，新旧A类executed显示真实边界。
