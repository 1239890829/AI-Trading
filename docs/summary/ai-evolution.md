...
> 当前实施与权限修订见 [实施校准 v9.1](../implementation-plan.md)，状态唯一归账本 §6.0；本页历史“自动转正/事后回滚”描述不构成新的线上参数授权，保留背景并由 IMP-046 核当前消费者。
...
- 运行默认：`ASHARE_AGENT_AUTONOMY_ENABLED=1`、`ASHARE_AGENT_CODE_CHANGE_ENABLED=0`。
  前者使 A/B 类受限自治与自动转正/回滚运行；紧急停机时设为 `0`。C 类仍要求代码开关显式开启，
  且其“执行”只产出 patch 与隔离分支，不合并、不推送。
