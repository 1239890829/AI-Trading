...
> v9.7 当前目标链见 ../implementation-plan.md、../product/product-closure-design.md 与 ../product/hunting-decision-design.md；Jev 语义协处理边界见 ../ai/jev-integration.md；盘前/盘中来源保留，页面/消息/模拟/复盘逐步共用机会及决定版本。原双家族断点继续作为事实证据，不因UI合并就假定已打通；控制台调试视图迁后台，用户风险结果保留。
