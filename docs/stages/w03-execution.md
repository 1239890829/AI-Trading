...
- **验证**：对实施候选 `1a15703`：完整后端 `4015 passed, 80 skipped`，随后 `pyflakes app tests scripts` 通过；前端 TypeScript、ESLint 通过，默认时区与 `TZ=UTC` 均为 `73 files / 695 tests` 全绿，Next.js 16.3.3 production build 通过；`PickCard` 定向 37/37；`git diff --check`、`public_repo_scan.py`、`workspace-hygiene.py` 均通过。未改变选股阈值、仓位参数、真实券商/权限或策略收益口径。
