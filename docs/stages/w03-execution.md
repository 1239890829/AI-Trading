...
- **下一步**：旧来源未给稳定关联时保留未知；上游删除旧 ID 或缺口仍超过 100 页时保留可见 gap，另核可审计的人工处置。历史消费面继续核 `board_surge.match_news_events`：它从当前 EventCard/Direction 取题材异动告警归因并把文字存入告警快照，但未存解释版本；须限定真实消费者与失败语义后再处理，不与本片简报改动混合。Jev 候选只在冻结样本和独立人工对照就绪后推进。未实现项不得用前七片通过冒充全项完成。
