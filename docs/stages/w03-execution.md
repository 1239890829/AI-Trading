...
- **下一步**：旧来源未给稳定关联时保留未知。上游删除旧 ID 或缺口仍超过 100 页时，先用准确水位只读探测并保存证据，gap 继续可见；需另定有依据的人工处置，不得凭 `page_limit`、空页或旧 ID 消失自动宣称完整。继续按实际消费者核对剩余历史决策版本引用；Jev 候选只在冻结样本和独立人工对照就绪后推进。未实现项不得用前九片通过冒充全项完成。
