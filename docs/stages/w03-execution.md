...
- **证据**：候选分支 `chatgpt/imp006-execution-snapshot`，最终代码候选 `a48c49f`（后端主体 `1a15703`，随后仅修正非 ready UI 价格身份）引入 `execution-facts-v1`：同日同场景同股 `decision_id` 稳定，`decision_version` 只由会改变判定的物质事实生成，通知发送/去重或单纯采样时间刷新不伪造新交易版本；动作时快照携 source/as_of/received_at/freshness，`stale/degraded/unavailable/unknown` 从命中降为明确拒绝。乱序晚到旧 notification snapshot 仍 append-only 保留供回放，当前读视图只按 `as_of` 选择较新版本，既不覆盖历史也不让页面倒退。
...
- **验证**：生产代码候选 `a48c49f` 的后端与已全量验证的 `1a15703` 相同：完整后端 `4015 passed, 80 skipped`，随后 `pyflakes app tests scripts` 通过；`a48c49f` 的 UI 修正后又重新执行 `PickCard` 定向 37/37、TypeScript、ESLint、默认时区与 `TZ=UTC` 两轮 `73 files / 695 tests`、Next.js 16.3.3 production build，均全绿；`git diff --check`、`public_repo_scan.py`、`workspace-hygiene.py` 均通过。PR #60 首轮准确 CI 的 frontend/docs 已绿，backend 仅因 `test_decision_propagation_rejects_ready_handoff_with_pending_merge_text` 错把“当前必须处于 READY”当测试前提而失败（4014 passed/1 failed）；`488594f` 已将该测试改为自行合成 READY 反例，整份 `test_doc_health_ledger_stages.py`、pyflakes、doc-health 复验全绿，需以其后最新 PR HEAD 的 required CI 为最终远端门禁。未改变选股阈值、仓位参数、真实券商/权限或策略收益口径。
