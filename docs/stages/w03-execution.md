...
- **第四片待发布（2026-09-24）**：`codex/imp048-flash-recovery` 从第三片合并后的 master 开工。U49 反证发现旧水位落在 20 页之外时常规轮询持续重复同一区间；同日上游只读实测 `sortEnd` 连续翻到第 21 页。新增默认只读、显式频道/旧 code/21–100 页界限的手动恢复脚本；应用及调度器停止后才可 `--apply`，先做忽略区 SQLite 一致备份，再沿原入库与覆盖门写持久水位。旧 code 不重叠、条目入库失败或水位并发变化时不得虚报闭环。隔离快讯测试 **28 passed**，本机全后端 **4306 passed / 80 skipped / 0 failed**，前端常规/UTC 各 **713 passed**，tsc、eslint、Next build、全量 pyflakes、文档体检、卫生和公开仓扫描通过。该片仍需 PR/CI 与准确 HEAD 发布；未在真实运行库执行补采，不把只读上游探测当历史完整性证明。
- **下一步**：第四片发布后继续核来源新 ID 撤回对旧 ID 的显式关联，以及各股方向需独立依据时的人工审定；旧来源未给稳定关联时保留未知，不按文本相似度臆造撤回。上游删除旧 ID 或缺口仍超过 100 页时保留可见 gap，另核可审计的人工处置。核剩余历史决策消费者的版本引用；Jev 候选只在冻结样本和独立人工对照就绪后推进。未实现项不得用前几片通过冒充全项完成。
