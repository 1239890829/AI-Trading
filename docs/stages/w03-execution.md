...
- **施工进展（2026-09-24，首片待 PR/CI 发布）**：隔离 SQLite 已复现同题更正仍读旧摘要、第二来源丢失、同来源改标题断链。`codex/imp048-event-lineage` 新增逐来源观察、内容哈希、来源发布时间/本机接收与可见时间、完整来源标的列表；同一来源条目 ID 的修订追加而不改写旧解释，未复核变体暂停活跃事件、标的池、板块异动归因与 LLM 辅助写入；详情可读观察及原解释的 observation ID。旧库升级不伪造旧观察。此处只记录开发事实，未合并前不算验收完成。
- **下一步**：首片经全量门禁、准确 HEAD 发布并合并后，继续复核/采纳修订的显式版本链、历史决策引用与 `available_at` 回放、断档补采水位、撤回关联和多标的方向/未知口径；Jev 候选只在冻结样本和独立人工对照就绪后推进。未实现项不得用首片通过冒充全项完成。
