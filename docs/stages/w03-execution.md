...
- **下一步**：继续核来源新 ID 撤回对旧 ID 的显式关联，以及各股方向需独立依据时的人工审定；旧来源未给稳定关联时保留未知，不按文本相似度臆造撤回。上游删除旧 ID 或缺口仍超过 100 页时保留可见 gap，另核可审计的人工处置。核剩余历史决策消费者的版本引用；Jev 候选只在冻结样本和独立人工对照就绪后推进。未实现项不得用前几片通过冒充全项完成。
