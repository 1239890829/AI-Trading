...
- **下一步**：旧来源未给稳定关联时保留未知。上游删除旧 ID 或缺口仍超过 100 页时，须在具备准确水位的目标库上先只读探测并保存证据，gap 继续可见；需另定有依据的人工处置，不得凭 `page_limit`、空页或旧 ID 消失自动宣称完整。继续按实际持久决策消费者核对剩余版本引用与当时可见时点，尤其核已判卡修订复核后的重新候选/预算语义；部署搁置期间不把本机旧库或新代码视为运行验收。Jev 候选只在冻结样本和独立人工对照就绪后推进。未实现项不得用前十一片通过冒充全项完成。
