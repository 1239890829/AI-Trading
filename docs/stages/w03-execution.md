...
- **验证**：最终代码候选 `a48c49f` 的后端与已全量验证的 `1a15703` 相同：完整后端 `4015 passed, 80 skipped`，随后 `pyflakes app tests scripts` 通过；`a48c49f` 的 UI 修正后又重新执行 `PickCard` 定向 37/37、TypeScript、ESLint、默认时区与 `TZ=UTC` 两轮 `73 files / 695 tests`、Next.js 16.3.3 production build，均全绿；`git diff --check`、`public_repo_scan.py`、`workspace-hygiene.py` 均通过。未改变选股阈值、仓位参数、真实券商/权限或策略收益口径。
