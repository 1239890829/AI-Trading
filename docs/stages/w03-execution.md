...
- **证据**：候选分支 `chatgpt/imp006-execution-facts`，纯代码提交 `98191409d53086f626aa1cc2224e1dfcae1a51d2` 引入 `execution-facts-v1`：同日同场景同股 `decision_id` 稳定，`decision_version` 只由会改变判定的物质事实生成，通知发送/去重及仅时间戳刷新不伪造新交易版本；动作时快照携 source/as_of/received_at/freshness，`stale/degraded/unavailable/unknown` 从命中降为明确拒绝。乱序晚到旧 notification snapshot 仍 append-only 保留用于离线重放；当前/页面只读视图按 `as_of` 选择较新版本，不覆盖或删除历史证据。
...
- **验证**：定向后端执行链全绿；完整后端 `4015 passed, 80 skipped`，随后全量 `pyflakes app tests scripts` 通过。前端 TypeScript、ESLint 通过；默认时区与 `TZ=UTC` 均为 `73 files / 695 tests` 全绿；Next.js 16.3.3 production build 通过；`PickCard` 定向 37/37；`git diff --check` 与 public-repo scan 通过。未改变选股阈值、仓位参数、真实券商/权限或策略收益口径。
- **送审边界**：以上是作者实施与自检，**不是独立审核/发布放行**。准确候选仍须走 PR、required CI 与网页独立代码审核；审核前不得把本项标 `已完成`，也不得据此提前开工 IMP-044/IMP-049/IMP-053。
- **下一步**：对准确 PR HEAD 独立复核 reference→snapshot→AlertEvent→paper order→UI 全链及非 ready 反例；若 APPROVED 且 required CI 全绿再合并并标 `已完成`。随后重算阶段门：若 BUG-020 真实交易时段条件仍未变为可行动，则 G1 下一阻断项为 **IMP-044**；若其条件已满足，G0 重新抢占。
