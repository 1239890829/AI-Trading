...
- **2026-09-25 四 worker 对照片（Linux 待复核）**：主干最近八次成功运行的 backend pytest 步骤为 130–162s、依赖安装为 23–26s，三项 required job 均在；`test_factors.py` 的四个全评估慢例集中在同一文件。相同代码树、本机隔离临时目录且顺序运行的全量对照为：3 worker / `loadfile` 4338 passed / 80 skipped / 77.16s；3 worker / `load` 同数 / 76.70s（收益不足以改变模块级夹具分配）；4 worker / `loadfile` 同数 / 68.68s；5 worker / `loadfile` 同数 / 68.18s（仅快 0.50s，增加内存与调度开销）。本轮选固定 4 worker，保留 `loadfile`、全部测试、pyflakes、frontend、docs 和发布门。Linux PR/master CI 的同口径耗时及通过数仍需实测；本机结果不能宣称云端或费用同比例节省。
