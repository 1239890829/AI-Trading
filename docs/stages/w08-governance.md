...
- **证据**：历史26a0c44到9d439c5批检查过跟踪源、原项目scripts/.github、LaunchAgents与进程命令；通过safe-trash成对移除旧脚本及44项专属测试、核SHA并做一次脚本恢复再退出。对应retirement.json及Git保留；不是本次新查，也不宣称整台机器不存在未知外部引用。
  ...
  - 2026-09-21 多窗口审计新增两项执行纪律：① `待条件` 任务必须在时间/事件条件出现时重新计算阶段门，BUG-020 已在真实交易日漏过一次条件窗口；同日晚间已落地 `scripts/ledger-runtime-selection.py`，以 stage 显式 `运行条件` + 北京时间 + 持久化交易日历零外呼重算，本地固定时点验证 09:00 可从静态 G4/IMP-052 抢回 G0/BUG-020、20:00 恢复静态 G4；日历未覆盖 fail-closed 为 unknown，且不改写 stage 静态状态。② 同一 task/sub-slice 在降级全权模式下仍只允许一个 active worktree/PR（single-writer），替代分支确定后先提炼独有差异再关闭/清理，避免 IMP-020 #88/#90/#91/#92 式并发竞争。
...
- **范围**：每个获准批复核依赖安装/测试/构建/返工与云端触发成本，检查旧优化退化；按用户要求持续优化 CI 关键路径，保持三 job 和全部门禁。
...
- **2026-09-24 后端 CI 并行片**：PR #124 HEAD `df9295d170d86df69d4d0e2b8a2c049da457fc8e` 合并为 `5a0ab083de0da469cad78ae9859509d69165dac1`；仅在同一 backend job 将 pytest 设为固定 3 worker / `loadfile`，并锁定 xdist/execnet，保留 pyflakes、frontend、docs 与发布检查。相同代码树本机串行 4309 passed / 80 skipped / 128.06s，2 worker 105.82s，3 worker 78.50s；PR CI `35972114896` 三 job 成功，后端 job 174s、pytest 137.63s、4309 passed / 80 skipped；合并后 master CI `35972636974` 三 job 成功，后端 job 186s、pytest 145.75s、4309 passed / 80 skipped。此前四次串行 CI 后端 job 285–299s、pytest 246–256s，两次新 Linux 结果显示 job 缩短约 35–42%，仍须后续批次复核 runner 波动及账单。作者自审 `DegradedRelease` 回执 `5810193236` 绑定准确 HEAD，临合并 `release_check.py` 两次返回 exit 0；不声称独立 Review。代码功能分支本地与远端已清理。
- **2026-09-25 四 worker 对照片（Linux 已复核）**：主干改动前八次成功运行的 backend pytest 为 130–162s、安装 23–26s；同树本机全量对照 3 worker / `loadfile` 77.16s、3 worker / `load` 76.70s、4 worker / `loadfile` 68.68s、5 worker / `loadfile` 68.18s，均为 4338 passed / 80 skipped。采用 4 worker 并保留 `loadfile` 与三项 required job；PR #152 HEAD `8a5b33c3c9c55b9f97c44c328fa7d7219e684150` 的作者 `DegradedRelease` 回执 `5832060298`、临合并两次 `release_check.py exit 0`、PR CI `36132578762` 三项成功，后端 pytest 94.43s / job 126s；合并为 `5bdbee799e76899a3c697e3f16a49168a66ad35c` 后 master CI `36132953572` 三项成功，后端 pytest 134.05s / job 177s，两轮均 4338 passed / 80 skipped。后一轮落在旧耗时区间内，尚不能证明 Linux 稳定提速或账单节省。另在隔离工作树验证了把两个重因子用例拆文件的方案：全量 79.46s，对照未拆版 75.80s，故撤销未提交；冷 `uv` 安装 22.74s，与现有 pip 19–26s 相近，热安装需要约 660MiB 缓存，故未增加安装器。功能分支本地和远端均已清理；后续继续看真实 runner 耗时，不削弱测试或门禁。
- **下一步**：正式实施片先定向收敛，再按实际变更范围核完整检查；纯规划只验资料，未来发布仍满足准确SHA的审核及完整CI。2026-09-21 晚间已完成审计遗留的运行态接收，并纠正 Next 15.5.25 跨仓进程误判；AI-Trading 后端受控同步/重启后 SQLite `integrity_check=ok`/39 表、30/30 scheduler running，前端在 Node 22.22.2 下 `npm ci`、695/695 tests 与 Next 16.3.3 build 全绿。PR #96 exact-head PR-CI 三项成功并合入 `de86c680`，但 post-merge master CI `35607351247` 的 backend 在 4,194 passed / 80 skipped 后于 `test_endpoint_smoke` teardown 报 `unmocked socket operation`；PR-CI 同 HEAD 曾绿，证明旧网络门禁存在时序/归因缺口。进一步把 raw socket fail-closed 提升到 pytest session 后，真实外呼被归到 `test_news_content::test_endpoint_degrades_on_fetch_failure` 与 `test_theme_catalog::test_stock_themes_api`：前者靠真实东财 DNS/网络失败验证 502，后者会真实请求题材板块 K 线。当前修复不改生产代码：新闻正文/题材 API 两测试在各自外部边界显式注入失败，endpoint smoke 仅保留 HTTP/requests/TDX 局部隔离；全会话 socket guard 记录 thread/args/stack 并把泄漏归到发生时点。#97 首轮 Linux PR-CI 又由该硬门抓出第三条此前未隔离路径：`test_api::test_minute_signals_and_decisions_contract` 与 endpoint smoke 的 `/api/market/minute-decisions` 会经 `minute_decisions.tdx_points → minute_backfill.tdx_minute_line_fallback → easy_tdx MacClient` 直连 `121.36.248.138:7709`；原 `ASHARE_TRADES_TDX_FALLBACK_ENABLED=false` 只管逐笔 TDX，不覆盖分钟 K。测试基线现把真正外部边界 `minute_backfill.fetch_tdx_minutes` 默认替为 fail-closed unavailable，专项 minute fallback 测试仍显式注入 fake，因此不削弱生产逻辑或 TDX 专项覆盖。新增 TDX 边界并补 `test_default_tdx_minute_source_is_offline` 非空守卫后，最终本地全量 backend 为 4,275 collected、4,195 passed、80 skipped、0 failures/errors；该结果只是本机证据，仍必须由更新 HEAD 的 Linux PR-CI 与 post-merge CI 复验。Python `pip check` 当前无 broken requirements；npm registry mirror 不支持 audit endpoint，故仍不能声称当前 npm audit=0。
