...
- **证据**：历史26a0c44到9d439c5批检查过跟踪源、原项目scripts/.github、LaunchAgents与进程命令；通过safe-trash成对移除旧脚本及44项专属测试、核SHA并做一次脚本恢复再退出。对应retirement.json及Git保留；不是本次新查，也不宣称整台机器不存在未知外部引用。
  ...
  - 2026-09-22 BUG-020 真实会话又抓到**调度回执与本机副作用不一致**：平台定时任务曾显示 enabled、到点后仍 `last_run_time=null`，但远端工具历史证明同一轮实际执行了取证、两次进程 restart、代码/测试/文档修改及 hotfix 重载；禁用任务只能阻止未来调度，已启动的长任务仍继续与人工接管重叠。当前已停用该自动续跑，并将本项目规则收紧为：会修改 Git/服务进程/生产运行态的长验收只允许一个可观测的人工 single-writer；在调度层能把 run receipt 与真实副作用可靠关联前，平台定时任务只可做提醒/只读通知，不充当进程级执行器。
- **处置依据**：人工触发已代替自动发送需求；继续开发互调增加权限/状态/维护成本。退出不等于自动链成功，废弃子目标不得重新派工。
- **下一步**：U50 已有真实 PR 验证；持续验证 U49/U50 的发现质量、误报与成本。条件激活第一纵切已机械化：每次 continue 先运行 runtime selector；目前只登记 BUG-020 的 `A_SHARE_OBSERVABLE_SESSION`，其它 `待条件` 仍保持 fail-closed，只有出现稳定、可确定判据才新增条件类型，避免把自然语言条件猜成自动授权。继续保持 single-writer，并在 merge 后把 branch/worktree/runtime status 分开核对。GX 不借治理跨门。
