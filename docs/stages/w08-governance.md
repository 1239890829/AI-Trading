...
- **验收**：新增独立 workspace hygiene gate（不要把主机文件系统强塞进 doc-health）：至少支持只读检查、白名单安全清理和外部/未知资产只报告；扫描面有反向/注入自证，能够发现本项目遗留的临时克隆、已结束测试沙箱、超期恢复副本和未分类大产物。每轮 handoff 前先把完整测试目录压缩成“紧凑证据 + 必要恢复入口”再清理可再生目录；未知/跨项目/运行中/脏工作树/业务数据默认 fail-safe 保留。删除 tracked/独有内容走 safe-trash；明确可再生且由本项目拥有的临时/缓存项可由门禁白名单直接清理，避免“移进同盘回收站但空间不降”的伪清理。
- **证据**：2026-09-20 网页侧只读盘点发现主仓约 10GiB，其中 artifacts/runs 约 3.2GiB；72 个 pytest/worktree 类测试沙箱约 2.926GiB，而现役跟踪文档没有指向这些嵌套测试目录，顶层任务目录另有日志/XML/JSON/manifest/delivery 等紧凑证据。另发现约 1.1GiB 前端构建缓存、约 818MiB node_modules、约 691MiB 后端 venv；前者属于可再生缓存候选，后两者是昂贵依赖环境，不能按“缓存”二字每轮机械删除。/private/tmp 中确认无活动进程引用的旧 v9.4/v9.5 验证克隆与项目专属 pytest/Jev 临时目录已清理约 1.47GiB；跨项目仓库、活动开发仓、业务 data/parquet、插件管理器目录均未动。
- **下一步**：Codex 在不跨越当前 G0 主门的前提下，把 GOV-026 作为一个不冲突 GX 伴随切片实现：先写资产分类/保留策略与负例测试，再实现 scripts/workspace-hygiene.py（名称可按现有脚本风格调整）及 handoff 收尾调用；首轮只清白名单可再生项和已具紧凑证据的测试沙箱，不自动删跨项目目录。随后复核 archive 现存候选，先迁移代码/文档中的活引用再逐件退休。
- **恢复**：tracked 文档/规则从 Git/PR 恢复；恢复资产在相关变更合并并完成一次准确版本复核前不得清；本机业务数据与外部资产不进入自动删除面。门禁误报时先停自动清理，只保留 report/check 模式，不扩大删除白名单。
- **分工**：网页负责判定保留/退出语义、账本与审计；Codex 实现扫描/清理脚本、测试和精确证据；用户无需逐轮提醒“删缓存”，只有所有权/跨项目/唯一恢复证据不确定时才需要人工裁决。
