...
- **阶段目标**：阶段账本单点维护、重大决策传播、开放世界创新发现/准入前治理、持续合理节省成本，并按实际用途迁出平台绑定内容；不重建平行任务系统或自动采用平台。
...
- **下一步**：PR #41 已合并且准确 HEAD 的 backend/frontend/docs CI 与 release_check 通过；后续每个正常获准任务持续验证短提示接手及传播核对。是否有活动业务派工只看最新 handoff；不再独立建设自动化，旧 ledger-readback-01 作废。
...
- **验收**：新会话可调用 ashare-innovation-radar 从最新 master 恢复现有基线并产出候选卡；外部宣传与已核事实分开，外部内容按不可信数据处理；SHORTLISTED/LAB 有 falsifier、基线、成本/隐私/许可/恢复以及 `last_verified_at/review_by/experiment_budget/stop_rules`；真实实验归已有 stage；重大采用触发 plan-registry §1.1；雷达同时支持问题驱动、前沿驱动、反证驱动并把 missed-signal 反馈回查询/来源；雷达自身有有用候选率、漏发现、噪声和成本复评，不能只积压 WATCH。
