...
- **证据**：历史26a0c44到9d439c5批检查过跟踪源、原项目scripts/.github、LaunchAgents与进程命令；通过safe-trash成对移除旧脚本及44项专属测试、核SHA并做一次脚本恢复再退出。对应retirement.json及Git保留；不是本次新查，也不宣称整台机器不存在未知外部引用。
  ...
  - 该缺口已由 PR #64 闭环：reviewed HEAD `dd75593e` 的 CI run `35507688071` 全绿，网页 Review comment `5749537877`；新版 release_check 在隔离真实检出中对 PR #64/该 HEAD exit 0 并回显 receipt id/author/time，缺回执/错 PR/旧 HEAD 有反向测试。merge commit `6cbe7467` 的 post-merge CI run `35508271019` 亦全绿。
- **处置依据**：人工触发已代替自动发送需求；继续开发互调增加权限/状态/维护成本。退出不等于自动链成功，废弃子目标不得重新派工。
- **下一步**：发布门 companion 已闭环，回 G1/IMP-044。长期继续用真实“继续→实施→审核→收口”验证 U49 的发现质量、误报与成本；若再出现流程记录存在但身份不可证的边界，只如实标限制，不用小号/旁路伪造隔离。GX 不借 U49 跨门。
...
- **下一步**：本项作为 GX 伴随治理，不改变当前 G1 `IMP-044`。未来被选为伴随切片时，先做“现有 owner/字段/消费者/证据”差距审计与最小 metadata/report 契约，只补无法从现有资产回答的问题；随后按领域把衰退/知识贡献/成本 A/B 接到既有报告，不一次性重构所有注册表。任何生产降权、阈值变化或策略晋级仍回对应 owner task 与原证据门。
