...
- **证据**：历史26a0c44到9d439c5批检查过跟踪源、原项目scripts/.github、LaunchAgents与进程命令；通过safe-trash成对移除旧脚本及44项专属测试、核SHA并做一次脚本恢复再退出。对应retirement.json及Git保留；不是本次新查，也不宣称整台机器不存在未知外部引用。
  ...
  - 2026-09-22 IMP-052 关闭后 U49 重算发现 selector **错误返回 `gate=null/task=null`**，但 G1/G4 等仍存在可行动 non-blocker；根因是 `derive_current_stage_selection()` 只收集 blocker，§5.9 的“当前门内先 blocker 后 non-blocker”在 blocker 全清时没有 fallback。修复后算法明确：任一 G0–G4 blocker 仍全局优先；全部 blocker 清零才回落到最低可行动 non-blocker 门，同门候选继续按 role→P0/P1/P2→门内序。固定 fixture 新增“同门 non-blocker 保留候选 / 高门 blocker 压低门 non-blocker / 无 blocker 回落最低 non-blocker”三组反例。基于 `master@44b2bb5` 的重算首选为 **G1/BUG-022**，同门候选 `BUG-022, IMP-048, IMP-040, RSH-031`；本治理切片不顺手施工 BUG-022。
- **处置依据**：人工触发已代替自动发送需求；继续开发互调增加权限/状态/维护成本。退出不等于自动链成功，废弃子目标不得重新派工。
- **下一步**：U50 已有真实 PR 验证；持续验证 U49/U50 的发现质量、误报与成本。每次 continue 先运行 runtime selector：条件激活只接受 stage 明示且可机械判定的 condition；静态调度先 blocker、blocker 全清后才 fallback 到最低 non-blocker，避免“安全阻断已清=项目假完成”的死端。继续保持 single-writer，并在 merge 后把 branch/worktree/runtime status 分开核对。GX 不借治理跨门。
