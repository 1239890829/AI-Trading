...
- **下一步**：从最新 `master` 只做一个最小纵切，执行顺序固定为 `决策归档 → AlertEvent + durable dedup + Feishu Outbox intent 同事务 → brief/watch-ledger/paper 等派生消费者 → 异步发送`。先引入向后兼容的类型化 Feishu 回执（accepted / explicit_rejected / unknown，旧 bool 消费者保持原行为），再让每票 buy-point card 在事件落库前构建并存入 `AlertEvent.snapshot.card`；同一事务创建既有 Outbox intent，彻底移除随后 `send_interactive()` 直发。不要在本片顺手迁其它事件来源。
