...
- **下一步**：解锁条件：此数值候选不晋级。近期两池虽完成逐日分页，响应不回显交易日，北交未获可核标签；巨潮仅补了公告源声称的部分历史披露时间，未核首次可见／修订与全量覆盖。近期 ST／停牌／上市日期覆盖已补，但逐日身份发布时间和 2016–2023 旧证券宇宙仍未证实。历史题材成员、新闻首次可见、临板/封板前分母、队列可成交证据仍不足。须取得有授权、可核来源时间／修订与费用／许可的这些缺失数据，才重开层级机制、污染清单、purged walk-forward、消融／多重检验和成本／fill／MFE／MAE；无可用数据时保留为`待条件`，不把保留段描述为干净盲测或把盘后原因用作前日特征。正式特征／战法增益仍按 RSH-026／IMP-020，Jev 语义效果仍须 RSH-030 独立人工 anchor。
