...
- **下一步**：PR #69 已交付“全漏斗 outcome 身份 + denominator gate + 跨版本自愈”。当前候选继续同一 RSH-026，新增 D1/D3/D5 交易日 horizon：目标日只认 `trade_calendar` 注入的真实交易日，未来日历不足就暂不创建而非猜自然日；盘后实时请求面为 `watch ledger ∪ D0 pending ∪ selected future overdue`，deferred 失败分母不扩大实时请求；selected future horizon 支持最近 30 天有界逾期补标，同一 symbol 日 K 只取一次，超过窗口仍保留 pending 供离线专项回补。跨日收益仍是 reference 轨成本调整代理，`realizable_return=false`，明确不是 shadow fill。后续仍需真实运行库覆盖盘点、MFE/MAE、首次封板/time_to_limit 与公司行动/停牌版本化标签。真实覆盖未核前不进入 IMP-020 效果结论，也不自动回写生产权重。
