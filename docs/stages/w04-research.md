...
- **下一步**：PR #69/#70/#71/#72/#74/#75/#76/#78/#79/#80/#81 与账本 PR #73 已闭环；#81 merge=`e3750cd`、post-merge CI #529 全绿，真实工作区已同步。下午真实验收已证明 backend 无需 GET 即会自动消费 durable snapshot：13:05/13:10 两拍 scheduler 0 failure、run_id 稳定，但 `records=0`，因此 symbol-level DB 仍无 candidate/hard_gate/rank 行。此时不能把“零行”解释成“市场零候选”，因为现有 schema 没有 run-level 事实；日志一旦丢失就无法区分合法零候选、主题映射全空、linkage 降级或归档断链。当前子片新增 append-only `OpportunityDecisionRun`：每个 intraday run 无论 0/N symbol rows 都记录 trade_date/as_of/strategy/feature/data_state/snapshot identity、theme/participant/candidate-audit 数、stage/decision counts、linkage/summary/caveats 与 evidence digest；run row 与 symbol/outcome 同事务，exact replay 幂等，同 run 不同 digest 拒绝合并。scorecard/summary/replay 同步消费 run ledger，0-record degraded run 也能阻断效果结论。actual shadow fill/entry-capture 继续由 IMP-053 拥有。run-ledger 候选已在真实运行库一致性副本验证：`d9e4→e1a7` 后旧 156,593 snapshot / 156,593 outcome / 48 revision 全保留且新 run 表不历史反填；再注入一拍 ready/0-record 结构只增加 1 条 run、0 symbol/outcome，replay/summary 可解释合法零候选。run-ledger 发布并完成真实线上 0-run/N-run 验收前，RSH-026 不关闭、不进入 IMP-020、不自动回写生产权重。
