...
- **下一步**：BUG-028、BUG-026、BUG-020 均已闭环，G0 已结束；当前主门推进到 G1 / BUG-029。潜伏形态是否有稳定收益仍须走 RSH-026 / IMP-020 的效果证据，不能把 point-in-time 正确性写成策略有效性。
...
- **下一步**：BUG-026 与后续 BUG-020 均已闭环，G0 已结束；当前主门为 G1 / BUG-029。D1/D3/D5、MFE/MAE、真实可交易退出与跨日效果仍归 RSH-026 / IMP-020，本片不据 D0 代理声称策略有效。
