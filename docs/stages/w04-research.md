...
- **证据**：e6f2917基点新增68项反例均判红；已修零成熟、非有限/缺项/越界数值、配置阈值校验；登记册暴露机器条款/待终审，决策台账不再把pass直接写为准入。最终全量3783 passed/78 skipped，新增78项且原测试无删除；前端本地/UTC各681/72及静态构建通过。精确版本与最后复验见handoff及本批回执。
- **下一步**：当前代码与消费者专项通过，本批完整本地检查已通过，待独立审阅及准确版本CI；新分支codex/research-admission-integrity继承e6f2917，旧分支不动。未运行真实数据研究或改写存量报告。
