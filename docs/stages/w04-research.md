...
- **下一步**：BUG-028 已闭环；后续 BUG-026 也已闭环，当前 G0 主切片已推进到 BUG-020。潜伏形态是否有稳定收益仍须走 RSH-026 / IMP-020 的效果证据，不能把 point-in-time 正确性写成策略有效性。
...
- **下一步**：BUG-026 已闭环；重算阶段门后 G0 下一主切片为 BUG-020。D1/D3/D5、MFE/MAE、真实可交易退出与跨日效果仍归 RSH-026 / IMP-020，本片不据 D0 代理声称策略有效。
