...
- **下一步**：PR #69/#70/#71/#72/#74/#75/#76 与账本 PR #73 已闭环；#76 merge=`340f7338`、post-merge CI #519 全绿，cross-day cumulative MFE/MAE 工程链已接入生产 EOD。真实长期工作区已同步到该 master，旧库 `d2e4a6b8c0f1` 受控迁到 `d9e4c2b7a1f6`，D0 identity 恢复为 156,108/156,108，追加 48 条 current revision，原 48 条 terminal labeled base 逐字段保持不变，二次 apply `noop=true/backup=null`；部署后 backend 全量 `4102 passed / 80 skipped`。启动后 2026-09-21 DailyPickSet 已自动生成，buy-point 后台链已自动归档 10 条 notification/rejected + 10 条 D0 deferred identity。由此反证出剩余工程缺口：candidate/hard_gate/rank 归档仍只在读取 `/api/picks/intraday-*` 时发生，前台 GET 不得充当业务触发器。当前子片把机会构建抽到共享 service，并按新的持久 market snapshot 版本由后台 scheduler 自主归档；同一 snapshot 的 run 身份绑定真实 snapshot `as_of`，GET 与 scheduler 重放保持幂等。actual shadow fill/entry-capture 继续由 IMP-053 拥有；RSH-026 未完成在线样本成熟前不进入 IMP-020 效果结论，也不自动回写生产权重。
