...
- **下一步**：最终第二子片真实 marketdb 隔离重跑：candidate B 48/48 trials 全量登记，selected Bonferroni `p_adj=0.531822444114` 未存活；PIT 由 build evidence 自动判为 true，holdout 与 two-thirty-five 交集 0，仍因 test-informed hypothesis family + 多重校正未过保持 `observe`。two-thirty-five 单 trial `p_adj=1.0`，与 candidate B `intersection=38 / union=42,305 / Jaccard=0.00089824`，非重复；但 `turn` 来自 current float-shares，feature PIT 自动判 false，且收益为负，保持 `reject`。现存测试段不能事后洗成 clean OOS；clean 证据只能来自预注册后的未来/外部未见样本。下一纵切做 leave-one-component-out 消融 + 同版同窗同成本 Champion/Challenger 与实验账本；actual shadow fill 净收益仍由 IMP-053 提供。
