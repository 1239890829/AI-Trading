...
- **下一步**：PR #69/#70/#71/#72/#74/#75 与账本 PR #73 已闭环；#75 merge=`a96c3d00`、post-merge CI #517 全绿。当前实施 **D1/D3/D5 cross-day cumulative MFE/MAE**：D0 只接受完整的决策后 Tencent 1m path，随后每个预期 future 市场交易日必须有与 price-basis 同源的 qfq daily high/low；future qfq extrema 按决策日 qfq/raw factor 映射回 raw-reference 等价尺度，再与只含决策后时段的 D0 raw extrema 合并；同一 `path_high/low` 列跨 horizon 不改变价格尺度。缺中间交易日 bar、日历不足、跨源或 D0 不完整均 fail-closed；旧长期库 `d2e4a6b8c0f1` 根本没有 path/MFE/MAE 列；其本地 marketdb 的 `daily_k_adj` 也只有 `close_adj`、没有复权 high/low，因此历史 cross-day path 不做伪造回填，只从有完整 D0 path 且在线 qfq OHLC 可验证的新样本开始积累。actual shadow fill/entry-capture 继续由 IMP-053 执行轨拥有。该片合入后重新计算 RSH-026 是否还有属于本任务的必要余项；RSH-026 未整体闭环前不进入 IMP-020 效果结论，也不自动回写生产权重。
