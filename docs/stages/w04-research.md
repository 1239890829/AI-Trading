...
- **下一步**：PR #69/#70/#71/#72/#74/#75/#76/#78/#79/#80 与账本 PR #73 已闭环；#80 merge=`bc102f56`、PR CI #525 与 post-merge CI #526 全绿，真实长期工作区已同步，30/30 scheduler 正常，质量门真实读侧已显示 9/16 ready run-symbol coverage=74.28%、今日 current-v2=35.48%。最终“无 GET 自动产生 candidate/hard_gate/rank”仍待第一份下午盘中 durable snapshot。发布后主动审计又发现 point-in-time 原子性风险：旧 shared builder 用 Parquet snapshot 做题材 premium，却在重型 IO 后重新读内存 snapshot 做 linkage/tradability，并在归档末尾再次读取 `last_success`；若构建跨过一次 market refresh，一条 run 会拼接 A/B 两个事实时点。当前子片在 cache key 生成时一次冻结 `snapshot_by + freshness state + as_of`，整轮 board/linkage/tradability/archive 共用同一 bundle；`MarketSnapshotService` 额外把成功落盘的 exact path/as_of 与 `saved_files` 原子绑定，后台 evidence scheduler 只消费触发它的 exact durable parquet，并按该 saved fact-time 判断盘中/盘外，禁止“cursor=A / 内存=B”与 11:29→11:31 消费时钟错杀；仅无内存快照的普通 route 冷启动才回退 Parquet，且 `snapshot_state!=ready` 或缺 `snapshot_as_of` 一律把归档 `data_state` 标 degraded。actual shadow fill/entry-capture 继续由 IMP-053 拥有；下午 live evidence 验收 + 本原子性门发布前，RSH-026 不关闭、不进入 IMP-020、不自动回写生产权重。
