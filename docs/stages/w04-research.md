...
- **下一步**：第三子片固定 incumbent/challenger 身份并加入同版 leave-one-out 与 Champion/Challenger 研究证据；旧隔离探针表明 registered candidate B 仍 observe、训练 challenger 虽更高但不可晋级，two-thirty-five 消融后整体仍 reject。本次适配 exact HEAD 需隔离重跑后再把具体数字升级为正式证据。第三子片完成后，最后纵切只收晋级准备态、actual shadow-fill 同版身份与 rollback/reopen；actual fill 缺失时必须 blocked，自动系统最多给 ready_for_human_review。
