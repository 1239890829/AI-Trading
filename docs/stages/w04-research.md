...
- **证据**：既有 append-only 快照、离线重放与 D0 标签沿用；PR #69 已把每个 funnel snapshot 接到同表 outcome identity，区分 selected `pending` 与 rejected/unknown `deferred/not_actionable`，新增 full-funnel denominator gate 与旧 run outcome 自愈；PR #70 已新增 D1/D3/D5 真实交易日 horizon、selected-only 30 天逾期恢复，并明确所有跨日收益仍是 reference 代理而非 shadow fill。当前子片进一步扩展**同一张** `OpportunityOutcomeLabel`：D0 selected 路径只用严格晚于 `snapshot.as_of` 的腾讯 1m 计算 high/low/MFE/MAE，决策当分钟保守排除，且目标交易日序列必须覆盖到 15:00 才可 `labeled`，截断前缀保持 `pending`；首次封板事实取最终涨停池 ∪ 炸板池，区分 hit/not_hit/preexisting/hit_time_unknown/unknown；新增 path source/version/state/reason/bar_count，旧行保持 unknown 不伪造 0。仓内反例测试已证明决策前/当分钟极值不泄露、炸板成员仍算 ever-hit、东财分钟源不会回退使用、terminal unknown 不重复外呼。当前远程 Mac 没有可核的长期生产运行库，因此仍**不声称**真实历史覆盖比例已测出。
- **下一步**：PR #69/#70 已闭环 denominator 与 D1/D3/D5。本轮先交付 D0 post-decision MFE/MAE + ever-hit-limit/time_to_limit，并保留 selected-only 实时分钟请求面；跨日累计 MFE/MAE、实际 shadow fill 路径/entry-capture、真实运行库覆盖盘点、公司行动/停牌版本化仍是后续 RSH-026 子片。真实覆盖未核且 RSH-026 未整体闭环前不进入 IMP-020 效果结论，也不自动回写生产权重。
