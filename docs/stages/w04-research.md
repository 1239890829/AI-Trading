...
- **下一步**：PR #69/#70/#71/#72 已分别闭环新写入 denominator、D1/D3/D5、D0 post-decision path 与历史 D0 全漏斗恢复；PR #72 merge=`726fdeb0`，post-merge CI #511 全绿。RSH-026 仍未整体完成。按正确性阻断优先，下一候选为**公司行动/停牌版本化**：先证明 D0/D1/D3/D5 与路径结果在除权除息、拆并股、停牌/复牌等状态下不会把机械价格变化或缺失交易日误当策略收益，再考虑跨日累计 MFE/MAE。actual shadow fill/entry-capture 继续由 IMP-053 执行轨拥有；正式旧部署恢复须在代码+schema 同步和停调度条件满足后单独执行。RSH-026 未整体闭环前不进入 IMP-020 效果结论，也不自动回写生产权重。
