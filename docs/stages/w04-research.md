...
- **下一步**：第三子片已固定 incumbent/challenger 身份并补同版消融、same-basis Champion/Challenger 与独立 append-only experiment ledger。登记 Candidate-B 35bps holdout 仍为 observe；训练 challenger 即使同基准超额更高，也仅是 `reference_close_to_close_proxy` 研究证据、`promotion_basis_eligible=false`。two-thirty-five 组件消融不改变整体 reject。最后子片只收 promotion-readiness / actual `shadow_fill_net` 身份校验 / rollback-reopen 合同：actual fill 缺失时明确 blocked；齐备时最多 `ready_for_human_review`，不自动上线。
