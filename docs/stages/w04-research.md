...
- **证据**：旧 ntile 修复和多窗口扫描已完成；IMP-026 的旧版本结论需复核，不能盲目沿用旧 40 因子数字。2026-09-21 `verification-protocol v1` Preflight 已实证：marketdb 10,298,838 行/5,565 symbol/2,438 交易日；旧 current-snapshot name filter 会删除 204 symbol / 452,804 历史行；旧 split 无 purge/embargo；两份正式 verify 产物 cost_bps 均为 0。第一子片已在 `strategy_verify` 建立验证协议底座；第二子片升级为 protocol v2 / gate v4，并建立结构化 trial-family + overlap 证据；随后 U49 hardening 进一步把 PIT 从 caller 布尔改为 `BuildConfig + gate_features` 派生，为 build/trial/overlap/protocol 加 canonical digest，并让 verify registry 保留 prior-current history、读回时重新校验 protocol：trading-session purge/embargo、35bps 研究压力成本、PIT universe/feature、试验全集 + multiple-testing、signal overlap、return identity 分离；current snapshot float-shares 改为 LEFT JOIN 诊断特征，不再用 current name/ST/退市删除历史 universe。真实 10 年 marketdb 只读探针验证带/不带 current 股本近似均为 10,187,702 特征行；2022 holdout 四段分母守恒，purge/embargo 各 5 交易日；协议缺陷会把 machine-pass 压为 observe，且 production promotion 恒为 false。
- **下一步**：第二子片已把试验全集/多重检验/重叠检查机器化：candidate B 48/48 trials 全量登记，选中规则 Bonferroni `p_adj≈0.532` 未存活；与 two-thirty-five/legacy candidate 当前事件集合不重复，仍因 test-informed + 历史身份非 PIT 保持 observe。two-thirty-five 单 trial、与 candidate B Jaccard≈0.0009，仍因负收益 + 非 PIT 保持 reject。下一纵切做 leave-one-component-out 消融 + 同版同窗同成本 Champion/Challenger 研究对照；actual shadow fill 净收益仍由 IMP-053 提供。
