...
- **下一步**：PR #69/#70/#71/#72/#74/#75/#76/#78/#79 与账本 PR #73 已闭环；#79 merge=`9c4dfc14`、post-merge CI #524 全绿。真实长期工作区已同步并恢复默认 backend：30/30 scheduler 运行，hotfix 后 `opportunity-evidence` 当前 0 failure；11:43 durable snapshot 落在午休，按 `outside_trading_window` 规则只消费 cursor、不归档，故最终“无 GET 自动产生 candidate/hard_gate/rank”验收仍待第一份下午盘中 durable snapshot。并行主动审计又发现评估质量风险：真实 9/16 有 1,916 条 degraded、今天当前有 180 degraded + 20 unavailable；旧 scorecard 只看标签/price-basis，非 ready 决策事实理论上可进入效果样本。当前子片保留全部 degraded/unavailable/unknown 行在 audit/全漏斗分母，但统计样本、成本代理和 Precision@K 只认 `data_state=ready`；只要完整漏斗中任一 run-symbol 的任一阶段非 ready，新增 `evidence_quality.complete=false`，即使标签全齐且样本数达标也以 `verdict=degraded_input` 阻断效果结论。actual shadow fill/entry-capture 继续由 IMP-053 拥有；下午 live evidence 验收 + 本质量门发布前，RSH-026 不关闭、不进入 IMP-020、不自动回写生产权重。
