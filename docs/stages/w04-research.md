...
- **U52/U54/U55子范围（P07/P08）**：依赖/派工满足后取一个已准入场景做同版机会读模型纵切，覆盖观察/等待/触发/actionable/失效、原文/KB反证、未知线索及未选原因；GET不生成业务样本或模型费用。参考轨、通知、H04影子和H05结果用引用连接，follow/pin/compare不改草稿。复核RECOVERY 54b7e0c与现实现，只有缺口仍在且证据支持才最小移植；不得盲cherry-pick。H02“机会发现”是B导航候选，开放情境与真实条件不由导航更名改变。
...
- **U54消费验收（P21）**：工程完成态保留，后续只用真实IMP-053产物验readiness consumer；缺actual fill/成本/退出证据时blocked是真实结果，完整时仅ready_for_human_review，不由新布局或JEV自动晋级。H05按reference/actual-fill及scope各自呈现，不再开一个无终点研究平台任务。
...
- **证据**：现有 theme-sentiment、theme-prediction、hunting-decision、KB37项及 strategy-registry 已含梯队、角色、消息、弱市穿越、结构转换和失败样本的碎片知识。2026-09-26 原 Phase 0 只读 marketdb 得探索段 676.6 万条日线分母、29.2 万条名义涨幅代理事件；原 2024／2025+ 当时未查询，记录见[研究蓝图 §14](../research/limit-up-dragon-research.md#14-phase-0-首轮实测与停止点2026-09-26)。用户随后允许用可得近期窗口；先冻结协议后，现有同花顺通道采得 2025-10-09 至 2026-08-31 的 220 日沪深源返回池：涨停 17,393、炸板 5,571，440 池分页/唯一性核对无报错。前日连续 20 日可观察分母 1,122,700 证券日、首板 13,117；固定组合在保留段 Top20 命中 57／1300，简单相对强弱为 61／1300，未证明增益。原始摘要、SHA、首次北交负例误含的修正和未通过项见[研究蓝图 §15–16](../research/limit-up-dragon-research.md#15-近期可得数据的预注册补充2026-09-26)。2026-09-27 免费来源实测见[§17](../research/limit-up-dragon-research.md#17-免费历史来源的定向实测2026-09-27)：巨潮历史公告披露时间与附件可查；BaoStock 对近期 5,214 只证券取得 1,138,688 行身份，原 1,122,700 证券日与 17,393 条涨停池 **0 缺失 ST、0 源间 ST 冲突**，含 35,445 ST 日、481 停牌日。仍未取得点时题材、首次可见文本或全量可成交证据；原评分未重算。这是源返回的近期沪深盘后事件，非全市场点时事件／交易效果通过。
- **下一步**：解锁条件：此数值候选不晋级。近期两池虽完成逐日分页，响应不回显交易日，北交未获可核标签；巨潮仅补了公告源声称的部分历史披露时间，未核首次可见／修订与全量覆盖。近期 ST／停牌／上市日期覆盖已补，但逐日身份发布时间和 2016–2023 旧证券宇宙仍未证实。公开分钟数据已核实单股实文件和覆盖表，但 2026-08-07 盘中截止，不能覆盖原保留段至 08-31，亦无订单队列。历史题材成员、新闻首次可见、全窗临板/封板前分母、队列可成交证据仍不足。须取得有授权、可核来源时间／修订与费用／许可的这些缺失数据，才重开层级机制、污染清单、purged walk-forward、消融／多重检验和成本／fill／MFE／MAE；无可用数据时保留为`待条件`，不把保留段描述为干净盲测或把盘后原因用作前日特征。正式特征／战法增益仍按 RSH-026／IMP-020，Jev 语义效果仍须 RSH-030 独立人工 anchor。
...
- **U52/U54/U55子范围（P06/P20）**：Phase0冻结历史宇宙、失败对照、污染地图与原文覆盖；数值增益仍经RSH-026/IMP-020。行业/市值/波动/制度/可交易池匹配基准，成本、流动性/容量、完整试验族、purged OOS和负结果不可省。结果H05阅读、H02按准入消费；研究运行在受控后端，菜单出现不产生执行或晋级授权。
...
- **U52/U54研究范围（P20）**：复用当前研究准入，按失败对照与真实未展开定义选择最小假设；全试验族、历史PIT、样本外/前向、同池基准、成本/容量/尾损与相关性消融齐备。H05展示负结果、成熟度与适用边界，不因增加情境/更名重启已否决方法，不把可视化建设当研究增益。
