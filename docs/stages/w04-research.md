...
- **证据**：旧 ntile 修复和多窗口扫描已完成；IMP-026 的旧版本结论需复核，不能盲目沿用旧 40 因子数字。2026-09-21 `verification-protocol v1` 已由 PR #86 合入：trading-session purge/embargo、35bps 研究压力成本、PIT universe/feature、return identity 分离，并消除 current-name/ST/退市过滤历史样本的泄露。第二子片继续把“声明”升级为**证据派生**：`build_validation_evidence` 从真实 BuildConfig/gate features 机械判 PIT；`strategy_trials` 保存完整 trial family、Bonferroni 校正与 digest；signal overlap 按 symbol×date 精确集合计算 Jaccard/containment/exact-duplicate；protocol/gate 不再接受 caller 自报 `*_point_in_time`、`multiple_testing_accounted`、`signal_overlap_checked` 布尔值。verify registry 改为保留 prior-current history，负结果与旧尝试不再被最新一次覆盖抹掉。
- **下一步**：第二子片真实 marketdb 只读探针（未覆盖生产 verify JSON）得到 10,193,239 特征行，2022 切分为 train 1,270 / purge 5 / embargo 5 / test 1,139 个交易日。candidate family 48/48 trial 全登记，训练选中 `grid-28` 的 Bonferroni `p_adj=0.5318`，未通过；与 two-thirty-five 的 holdout 事件交集 0，因此当前阻断项为 `test_informed_hypothesis_family + multiplicity failure`，而不是“overlap 未查”。two-thirty-five 单一预注册 trial 的 `p_adj=1.0`、未通过；与 pullback_reversal holdout 仅交 38 个事件（Jaccard 0.000898、target containment 0.00581），但 `turn` 仍由 current float-shares 反推，feature PIT=false。以上只证明验证链能真实否证，**不证明策略效果提升**。下一纵切只做消融 + 同版 Champion/Challenger/试验账本；第四纵切收晋级纪律与回退闭环。actual shadow fill 净收益仍由 IMP-053 提供，不并入 IMP-020 伪造。
