...
- **证据**：旧 ntile 修复和多窗口扫描已完成；IMP-026 的旧版本结论需复核，不能盲目沿用旧 40 因子数字。2026-09-21 `verification-protocol v1` Preflight 已实证：marketdb 10,298,838 行/5,565 symbol/2,438 交易日；旧 current-snapshot name filter 会删除 204 symbol / 452,804 历史行；旧 split 无 purge/embargo；两份正式 verify 产物 cost_bps 均为 0。第一子片建立 purge/embargo、35bps、PIT/return identity 底座；PR #87 将第二子片推进到 protocol v2 / gate v4 和结构化 trial-family + exact symbol×date overlap。随后 U49 加固继续封死三类假证据：PIT 不再接受 producer 手填布尔值，而由实际 `BuildConfig + gate_features` 机械派生；trial/overlap 证据带 canonical digest，trial 的 p 值/Bonferroni/selected-survival 会从完整 manifest 内的训练 n/excess/std 重算，重新封 digest 也不能伪造；合法旧 v2/v3 gate 只保留为 `legacy_protocol_unverified`，verify current 覆盖前把上一版 current 追加到 history，负结果与多次尝试不被无痕抹除。current snapshot float-shares 仍只作 LEFT JOIN 诊断特征，不用当前名称/ST/退市删除历史 universe；真实 10 年 marketdb 隔离验收未覆盖仓库历史 JSON。candidate B 48/48 trials，训练选中规则 Bonferroni `p_adj=0.531822444114`、未过阈值；35bps purged holdout 中性 +0.19%、中性中位 -0.80%、跑赢 44.9%、年度正 5/5，和 two-thirty-five 精确交集 0，仍因 test-informed 假设族保持 observe。two-thirty-five 单预注册 trial `p=1.0`，holdout 中性 -0.99%、中位 -2.42%、跑赢 33.9%、年度正 1/5；`turn` 的 current-float-shares 回推仍非 PIT，与旧 pullback 交集 38/union 42,305（Jaccard≈0.000898），保持 reject。`production_promotion_eligible=false` 恒成立。
- **下一步**：第二子片在本加固补片合入后视为收口；子片③做 leave-one-component-out 消融、同版同窗同成本 Champion/Challenger 与完整 experiment accounting，确保版本、对照、负结果和所有尝试可追溯；子片④再收口晋级审批、生命周期与安全回滚。actual shadow fill 净收益仍由 IMP-053 提供；四个子片全部完成后才关闭 IMP-020 并重算阶段门。
