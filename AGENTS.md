...
- 接续工作：`skills/ashare-ledger-continue/SKILL.md`；当轮交接：`skills/ashare-task-handoff/SKILL.md`。总账 `docs/retro-and-gaps.md` §5.9 的 G0–G5/GX 是唯一执行门序；每次“继续”先运行 `scripts/ledger-runtime-selection.py` 重算 stage 明示的受支持 `运行条件`。G0–G4 仍有任一可行动 blocker 时先取**最低 blocker 门**；全部 blocker 清零后才回落到**最低可行动 non-blocker 门**，再按角色/P0-P2/门内序领取一个切片。运行条件只临时影响本轮选择，不改写 stage 静态状态；硬依赖未完成不得跨门；`CROSS_GATE_EXCEPTION` 只能由网页在 handoff 明示。
