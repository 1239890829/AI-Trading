...
> **最新门禁口径（2026-09-17 IMP-041，本地完整验收）**：后端 **3566 collect = 3490 passed + 76 skipped，0 failed**；
> 前端本地/UTC 均 **650 passed / 70 文件**；tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
> 后端 **141.26s**：8000/3000 均未运行，后端与前端全量串行，期间有只读核查及文档编辑；前端单 worker **54.86s / 45.03s**。
> 题材基线 `7f635d7` 与本批 **3566 / 216 文件** 及全部用例身份相同；import-lint 均 **269**、skipped 均 **76**。15 处装载替换之外测试 AST 相同，9 类夹具表结构与逐值哈希相同。
> doc-health **23 条交接索引 / 0 档位冲突**；完整 OpenAPI **170 paths / 零差异**。用户已恢复剩余额度使用；本批与跌停/题材保护集中进入 PR #24，准确新 HEAD 的 CI 与节省分钟仍待实测，现场与预算见 `docs/handoff.md` §1。
>
