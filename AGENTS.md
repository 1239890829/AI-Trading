...
> **最新门禁口径（2026-09-17 IMP-043 / BUG-023 / BUG-012 合批，本地完整验收）**：后端 **3670 collect = 3594 passed + 76 skipped，0 failed**；
> 前端本地/UTC 均 **675 passed / 71 文件**；tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
> 后端 **154.87s**：8000/3000 停用，前后端全量串行，期间有静态检查与只读取证；收尾负载 6.60/6.94/5.91，不与空载耗时直接比较。前端单 worker **94.41s / 74.31s**，本地轮期间另跑 tsc/eslint。
> 基线 `b5a9f7b`：后端 **3622 → 3670 / 220 → 222 文件**，原用例身份全部保留；新增 **48** = 回执 **39** + 网络隔离 **8** + 后台议程隔离 **1**，skipped 76 不变。前端全部 675 项身份不变。
> 原冒烟/个股资金流断言全部保留；同机隔离对照 141 条冒烟 **20.73→2.97s**，网络尝试 **42→0**，不当作全 CI 比例。曾红的三项共享库失败另归 BUG-012，修复后全量通过；其它并发来源仍开放。持续分钟复核归 IMP-043，通知 Outbox 归 IMP-044；权威证据与预算见 `docs/handoff.md` §1。
>
