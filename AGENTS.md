...
> **最新门禁口径（2026-09-17 BUG-020 指数缓存缺失，本地完整验收）**：后端 **3574 collect = 3498 passed + 76 skipped，0 failed**；
> 前端本地/UTC 均 **651 passed / 70 文件**；tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
> 后端 **186.05s**：8000/3000 未运行，前后端全量串行；期间有只读核查、隔离 ASGI 与 OpenAPI 导入，宿主负载抽样 6.76/5.49/5.82；前端单 worker **101.69s / 67.60s**，不能与空载耗时直接对标。
> 基线 `f6182c6` 的 **3566 / 216 文件** → 本批 **3574 / 217 文件**；全部 **+8** 为新指数回归，旧用例身份无删改，import-lint **269**、skipped **76** 不变；前端新增 1 项质量显示回归。
> doc-health **23 条交接索引 / 0 档位冲突**；完整 OpenAPI **170 paths / 零差异**。PR #24 已合并（跌停/题材保护/CI 装载优化），本指数切片先保留本地验收提交、按完整批次安排云端；交付身份与预算见 `docs/handoff.md` §1。
>
