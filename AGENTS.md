...
cd backend && .venv/bin/pytest --basetemp=/tmp/pytest-basetemp     # 后端 2584 项（2523 passed / 61 skipped）· 168 文件（09-12 上午实测）
...
> **门禁口径**：后端 2584 项（2523 passed / 61 skipped）· 168 文件、前端 415 项 / 51 文件、eslint **0 error / 0 warn**
> （25 条回归已按 P1-27 清零；仅 notification-drawer 保留 1 处带理由的 C 类豁免）。
> **测试规模与告警数同属「会失真的状态标注」**——改动后要实测回填，不要沿用旧数字
> （此前「≤1 warn / 后端 580 / 前端 97 / 219 / 257 / 263 / 342 / 1925 / 2553 / 2556 / 2557 / 2574 / 2576 / 392」均已被后续改动追过，教训见 `docs/retro-and-gaps.md` §七）。
