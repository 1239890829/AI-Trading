...
cd backend && .venv/bin/pytest --basetemp=/tmp/pytest-basetemp     # 后端 2720 项（2658 passed / 62 skipped）· 174 文件（09-13 实测）
...
> **门禁口径**：后端 2720 项（2658 passed / 62 skipped）· 174 文件、前端 454 项 / 54 文件、eslint **0 error / 0 warn**
...
> 2584 / 2585 / 2590 / 2602 / 2606 / 2619 / 2620 / 2632 / 2635 / 2674 / 2691 / 2695 / 2708 / 前端 423 / 429 / 444」均已被后续改动追过，教训见 `docs/retro-and-gaps.md` §七）。
...
| 2 | 定时 automation 去留 | ⚠️ **原记载已失真，先纠正再决策**（2026-09-12 实测）。文档此前写「4 个定时 automation（09:26/14:40/15:35/15:40）是否一并停」，**这四个已不在实体中**。**实测实体（`automation_update list`，2026-09-12）共 4 条，其中 3 条活跃循环 + 1 条已过期一次性**：<br>① 周一 09:20「GitHub 仓库持续发现（每周，经验闭环）」（`1019920c`）<br>② 周日 10:00「仓库跟踪周报」（`ba64a0ef`）<br>③ 工作日 15:45「进化总结报告（每日唯一飞书推送）」（`49881f07`）<br>④ 一次性「P2-21 后端冷启动实测 + lurk_pool 陈旧披露线上验收（2026-09-11 午休）」（`58209806`）—— **`scheduledAt=2026-09-11T12:05` 已过期，但状态仍是 `ACTIVE`**。<br>**待决策**：①上述 3 条循环中**是否停掉部分**（减少打扰 vs 保留兜底）？②过期的那条一次性**是否清掉**（留着会一直占位且状态失真）？<br>**说明**：推送口径已是「盘中只保留买点卡」（09-08 定稿，飞书侧收敛完成），故本项**不再是「飞书推送去留」**，而纯粹是「这三条后台任务还要不要跑」。 |
