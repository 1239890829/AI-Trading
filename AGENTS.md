...
- 施工取舍：用户指定的最终融合方案及最新指令优先；旧项按账本 §6.0「方案准入」复核，登记不等于必须实施。
...
> **最新门禁口径（2026-09-17 GOV-019，本地完整验收）**：后端 **3716 collect = 3638 passed + 78 skipped，0 failed，129.21s**；前端本地/UTC **675 passed / 71 文件，46.52s / 49.14s**，单 worker。tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
> 8000/3000 停用、前后端全量串行；后端期间有文档与静态检查，宿主负载抽样 17.88/8.95/5.74 → 7.89/8.63/6.11，非性能对照。相对 `d4ebf94` 应用与测试零改动，后端 collect/passed/skipped 及前端数量均不变；doc-health **31 条交接索引 / 0 档位冲突**。本轮未触发 CI，待与实质切片合批交付，现场见 `docs/handoff.md` §1。
>
> **历史门禁口径（2026-09-17 IMP-044 / BUG-024 / GOV-018 合批）**：后端 **3716 collect = 3638 passed + 78 skipped，0 failed**；
...
> （P0/P1/P2，**与状态正交**），表内排序只用于检索；施工先过 §6.0「方案准入」，再按方案依赖与价值排期。**旧代号保留为「旧代号」列**
> （A1 / E1 / R03 / P1-42 / #5 …），既有引用仍可检索。
> · **要施工** → 按最终融合方案复核 §6.0 候选及当前入口；历史已交付项不重复领取；
...
> **按最新授权持续推进**：用户已批准在最终融合方案边界内分阶段实施，并允许充分论证后调整既有机制、设计与流程；旧规则不能代替当前准入判断，动手前先读 §6。
> **授权边界**：真实交易、数据真实性与撮合硬拦截红线不变；策略/资金口径、凭据及实际删除先核当前授权。已批准的清单内可恢复迁移不重复请示，未知资产与超出授权的动作不能据一般改进授权擅自处置。
