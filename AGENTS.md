...
- 施工取舍：最新用户要求与 docs/implementation-plan.md 的 v9.2 明确修订优先；原 v9 未修订部分保留，旧项按真实价值复核，登记不等于必须实施。
...
7. **禁止应用内自我修改代码并落地**（2026-09-15，审计 O1 / [[KB-DEC-026]]）：`app/services/code_executor.py`
   ...
   **不得合并回主分支、不得推送**；落地一律走 `codex/*` 分支 → PR → 完整 CI → §6.5 发布核验
   ...
   即使禁止改 tests，pytest 导入被修改的 app 仍会在宿主执行代码；现有关闭/不合并边界不是 OS 隔离证明。应用内执行隔离与参数晋级残余由 IMP-046 先行核验，未验证前只保留纯提案，不开放自动执行。
...
执行用户指定的最终融合方案 v9 及配套附件，并应用 docs/implementation-plan.md 的 v9.2 明确修订；最新用户指令优先。旧账本只提供问题证据，不自动产生施工义务；项目机制、流程、交接、设计及架构均可审视和调整。
...
2026-09-18临时代执行：用户因Codex额度不足要求ChatGPT继续按方案实施；前轮BUG-025已本地完成，本轮在继承分支上推进BUG-027并核对原任务要求，不调用Codex。长期分工保留，作者自检不冒充独立审核。用户允许比较无CI合并方案，本轮选择远端功能分支保存、待独立审阅与完整CI后再合并，不把尚未验证当作上线。

首次开工、审核整改与合并按协作规范 §3.1 分别校验；明确派工可授权首次实施，CHANGES_REQUESTED可授权限定整改，但两者都不代替成果审核或合并CI，避免“未开工就要求已审核”的循环。

2026-09-18 最新规则：方案、优先级、任务增删及验收标准由网页 ChatGPT 统筹；Codex可回填执行事实、证据和阻塞，并提出改进建议，但不能自行改规划或批准自己。每轮完成先提交审核包，收到绑定当前版本的有效网页审核与明确下一步后才继续。必要独立复现属于审核，不把业务代码实现转交网页侧。
...
禁止整包改名、直接删除未知内容或遗忘本机未跟踪资产；许可/保留期不清的内容先隔离保留。物理清理走 `scripts/safe-trash.sh` 可恢复；复核恢复不得覆盖现有目标。账本改造不能冒称平台目录已退出。
...
部署已由用户搁置；付费数据源不接入；真实券商禁止。自定义告警入口与模拟风控硬拦截保留。死代码扫描只产候选，不凭低调用量删除安全、迁移、恢复能力。
...
- `master` 是主分支；2026-09-17 实测平台保护未启用（`protected=false`），平台设置条件归 W00 的 `GOV-012`。开发任何功能、修复或重构前，必须先同步远程最新的 `master`；
