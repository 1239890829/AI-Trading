...
> **最新门禁口径（2026-09-17 IMP-042 与指数缓存集成，本地完整验收）**：后端 **3578 collect = 3502 passed + 76 skipped，0 failed**；
> 前端本地/UTC 均 **651 passed / 70 文件**；tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
> 后端 **141.36s**：8000/3000 停用，前后端全量串行；期间有只读核查、OpenAPI 导入与 pyflakes，负载抽样 3.41/3.54/3.41；前端单 worker **57.28s / 61.03s**。
> 基线 `a36946e` 的 **3574 / 217 文件** → 本批 **3578 / 218 文件**；全部 **+4** 为缓存热态回归，旧用例身份无删改，import-lint **269**、skipped **76** 不变；前端 651 用例身份全部相同。
> 完整 OpenAPI **170 paths / 零差异**；三项 CI 触发与命令保持完整。当前本地验收通过、集中 PR 交付待核验；权威证据与分钟预算见 `docs/handoff.md` §1。
>
