...
> 导航已收敛为 5 项。**当前剩余待办一律以 `docs/retro-and-gaps.md` §六 为唯一账本**——
> **「还有什么没做」的答案 = §六 的三处结转，缺一即漏**（2026-09-14 **两轮**复查后更新）：
> · **§6.5b** — 上一轮账本任务结转（9 项，绝大部分已闭环；余「等时间」与「搁置」）
> · **§6.6** — 全面审查 **R01–R26** 结转（23 行：R03 仅部分 / R08 迁库 / 分位 tie-break / 冷却档位 / 观察项等）
> · **§6.7** — 全仓计划/方案文档复查**补登记（两轮共 42 项）**：**A** 可排期 9 · **B** 待复核 19 · **C** 本轮建议 3
>   · **E** 二次补登记 **12**（同一份审查报告的 **F 系列功能机会 / O 系列架构问题 / B0–B5 批次 / §六验收项**
>   ——首轮按「未做/待…」关键词扫面时**整块漏掉**，含 **P0 级**的「R01/R02 历史污染审计与重算」）
> ⭐ **要按优先级施工 → 直接看 `§6.7-F`**（跨三处结转的唯一排序表：立即可做 / 必须先拍板 / 等时间 /
>   先复核 / 挂账搁置）。**三处结转内部都按「来源·轮次」组织、逐条只有状态标记、没有优先级字段**——
>   这是 2026-09-14 用户追问「是不是按优先级排序了」后暴露的缺口。
>
> ⚠️ **本节原写「唯一答案 = §6.5b」是失真的**——它漏掉了 §6.6 / §6.7 两处结转。此类「单一指针」
> 正是 2026-09-14 用户问「是不是还有没归纳的」时暴露的根因。**第二重教训（同日复查追加）**：
> 核覆盖面不能只看「**某来源是否已被覆盖**」——同一份产出物内有多类内容（R / F / O / 批次 / 验收项），
> **必须按「产出物 × 类别」双轴逐类点名**，否则会像首轮那样**漏掉 F 与 O 系列共 12 项**。
> §6.3 P2 全为触发式，**维持观察、不主动做**。
