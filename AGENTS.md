...
cd backend && .venv/bin/pytest --basetemp=/tmp/pytest-basetemp     # 后端 2780 项（2718 passed / 62 skipped）· 182 文件（09-13 §6.25 实测）
...
> **门禁口径**：后端 2780 项（2718 passed / 62 skipped）· 182 文件、前端 456 项 / 54 文件、eslint **0 error / 0 warn**
...
> **「还有什么没做」的唯一答案 = §6.5b**（2026-09-13 §6.18 全量执行轮后：**只剩等时间类**
> （P1-30 / P1-7+P2-11 / P1-23 / P2-14 / P1-42 解锁在即）**与搁置项**（部署）；
> 等拍板与挂账两类已清空）；§6.3 P2 全为触发式，**维持观察、不主动做**。
