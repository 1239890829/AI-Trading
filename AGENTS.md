...
> **最新门禁口径（2026-09-17 BUG-020 东财指数响应身份，本地完整验收）**：后端 **3596 collect = 3520 passed + 76 skipped，0 failed**；
> 前端本地/UTC 均 **651 passed / 70 文件**；tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
> 后端 **164.42s**：8000/3000 停用、前后端全量串行；期间有隔离 ASGI 验收与旧工作区归档，负载抽样 3.15/3.28/3.34；前端单 worker **75.75s / 88.18s**，期间有只读核查、文档回填和账单页面操作。
> 基线 `e976c10` 的 **3578 / 218 文件** → 本批 **3596 / 219 文件**；全部 **+18** 为东财 HTTP/Hub 回归，旧用例身份无删改，import-lint **269**、skipped **76** 不变；前端源码零差异、651 用例身份全部相同。
> 完整 OpenAPI **170 paths / 零差异**；PR #25 已合并且 master CI 通过，本东财切片待集中交付。权威证据与预算见 `docs/handoff.md` §1。
>
