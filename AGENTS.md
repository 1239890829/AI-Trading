...
> **最新门禁口径（2026-09-17 BUG-020 题材写保护，本地验收）**：后端 **3566 collect = 3490 passed + 76 skipped，0 failed**；
> 前端本地/UTC 均 **650 passed / 70 文件**；tsc **0**、eslint **0/0**、pyflakes **0**、生产构建通过。
> 后端 **190.74s**：8000/3000 均未运行，后端与前端全量串行，期间有只读核查与独立 SQLite 的 API 验收；前端单 worker **44.50s / 46.06s**。
> 基线 PR #24 实测 **3546 / 215 文件** → 本批 **3566 / 216 文件**；**+20** 全来自题材写保护回归，import-lint 两侧均 **269**、skipped 不变。
> doc-health **22 条交接索引 / 0 档位冲突**；完整 OpenAPI **170 paths / 零差异**。用户因 Actions 已用 90% 明确暂停新增运行：PR #24 CI 绿但未合并，本批仅保留本地提交，现场见 `docs/handoff.md` §1。
>
