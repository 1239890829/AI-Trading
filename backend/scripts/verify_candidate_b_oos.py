...
（前视偏差，仅作结构诊断）；历史 universe 不按当前名称/ST/退市状态过滤。
...
def main() -> int:
    ...
    trial_rows = [
        {
            ...
            "train": {"n": g["tr"].get(f"n{H}"), "excess": g["tr"].get(f"x{H}"),
                      "std": g["tr"].get(f"s{H}")},
        }
        for i, g in enumerate(grid)
    ]
    selected_trial_id = next(row["trial_id"] for row in trial_rows if row["label"] == main_name)
    trial_evidence = st.trial_family_evidence(
        trial_rows, selected_trial_id=selected_trial_id,
    )
    ...
    path = vr.save_record(
        ...
        sample={
            ...
            "caveat": "候选B 假设族曾使用全样本（含测试段）发现，不能追认 clean OOS；48-trial Bonferroni 与既有信号重叠已机械登记，但历史窥探仍阻断准入",
        },
