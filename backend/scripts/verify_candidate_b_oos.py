...
def main() -> int:
    ...
    print("【6】结论登记")
    print("=" * 94)
    registered_grid = next(g for g in grid if g["label"] == CANDIDATE_B_LABEL)
    registered_trial_id = next(
        f"grid-{i + 1:02d}" for i, g in enumerate(grid) if g is registered_grid
    )
    ...
    protocol = sv.validation_protocol(
        ...
        # The registered Candidate B family was historically test-informed; no retrospective wash to clean OOS.
    ...
    path = vr.save_record(
        ...
        sample={
            ...
            "limit_up_share": incumbent_lu.get("limit_up_share"),
            "caveat": "登记册 Candidate B 曾使用全样本（含测试段）发现；训练 grid 最优仅作 challenger，不覆盖 incumbent identity",
        },
        source="scripts/verify_candidate_b_oos.py",
        extra={
            "gate_failed": gate["failed"], "gate_unchecked": gate["unchecked"],
