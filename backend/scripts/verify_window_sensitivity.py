...
- 只跑**有效性 + 稳定性 + 覆盖率**三层，**不跑五分位分层**。主因是**成本**：分层要额外一次
  `ntile(5)` 全表扫描。
- ⚠️ **2026-09-15 更新（范围不变，但理由变了）**：本脚本最初**还**有一条次因——`ntile` 无
  tie-break 属已知缺陷（账本 `BUG-002`，并行下分位归属会抖）⇒ 用它当判据会引入噪声。
  该缺陷**已修复**（`ORDER BY f, thscode` 补唯一 tie-break，另加两条守卫）。**但本脚本仍不跑
  分层**：成本这条理由不因修复而消失，且把分层并进来需重新自证、重新标定容差，属**另一次改动**，
  不夹带在本轮。
