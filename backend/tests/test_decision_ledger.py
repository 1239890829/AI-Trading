...
def test_bug027_verification_view_never_conflates_machine_pass_and_admission(monkeypatch):
    from app.research import strategy_trials as st, strategy_verify as sv, verify_registry as vr
    trial = st.trial_family_evidence([{
        "trial_id": "one", "label": "one", "condition": "chg > 0",
        "train": {"n": 500, "excess": 1.0, "std": 4.0},
    }], selected_trial_id="one")
    overlap = st.overlap_evidence_from_counts(
        target_label="one", target_condition="chg > 0", target_n=10,
        comparisons=[{"incumbent": "inc", "condition": "vr > 1", "incumbent_n": 8,
                      "intersection_n": 2, "union_n": 16}],
    )
    protocol = sv.validation_protocol(
        ...
        selection_scope="train_only", build_config=sv.BuildConfig(), gate_features=("chg",),
        trial_evidence=trial, overlap_evidence=overlap,
    )
