...
5. 证据不足时弃权（空数组 / 少答）是合法输出，不要为凑结论编造数字。
   ...
   "仓位 N%"）的条目会被逐条丢弃。
6. 规律化表达必须可证伪：写成"若 <量化触发条件> 则 <结论>"并附适用边界与
   验证状态（未验证 / N 次验证），禁止输出不可证伪的规律（如"主力实力强"）。
   与既有知识库条目（docs/kb/，KB-ID 制）冲突时**显式写出冲突点**，不得静默
   覆盖——矛盾本身是有价值的信息。以上纪律出自 docs/kb/06-review-framework.md
   （复盘执行框架 v1.0），做单案例深度复盘时按该框架七阶段执行。\
"""
