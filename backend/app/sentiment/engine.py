...
根因在 `docs/strategy/sentiment.md`「历史误判案例库」已完整定位，并已修复。
...
阈值取自公开实战口径（见 docs/strategy/sentiment.md「历史误判案例库」多源交叉），
...
# --- 相位语义集合（S2-7，2026-09-11）——唯一权威，其余模块一律 import ---------
#
# **为什么必须集中**：相位字符串此前在 9 处被重复列举（5 个常量 + 4 处 inline），
# 且已经真实漂移出两处缺陷：
#   · `predict/engine.py` 的 `PHASE_ENV_SCORE` 与 `services/experiments.py` 的
#     `_SHADOW_PHASES` 里都写着 **「启动」**——那是**题材阶段**（theme_service.
#     judge_theme_stage 的产出：启动/发酵/高潮/分歧/退潮），不是市场相位。
#     后果：`启动` 是永不命中的死键，而真正的 `修复` 相位**从未被覆盖**——
#     预测引擎对修复期一直走 `.get(phase, 0.5)` 默认值，影子权重的逐相位
#     校验也从来没查过修复期。两套概念混用的代价是静默的。
#   · 相位是**交易信号级**输入：闸门按「退潮/冰点」撤买入区间、仓位引擎按相位
#     给分。任一处集合打错就是同一天两个模块对同一相位给出相反动作。
#
# 刻意**显式书写**而不是从 PHASE_ORDER 按下标切片派生：强弱两组相位分别位于
# 序列两端（冰点在头、退潮在尾），切片写法既不直观也经不起序列重排。
# 一致性由 `tests/test_phase_constants.py` 守卫（三组互斥且并集 == PHASE_ORDER）。

#: 弱势相位：赚钱效应最差，防守/空仓语义
ADVERSE_PHASES = ("退潮", "冰点")
#: 极端相位：ADVERSE_PHASES 的真子集，市场几乎无机会
SEVERE_PHASES = ("冰点",)
#: 强势相位：情绪策略最顺，进攻语义
