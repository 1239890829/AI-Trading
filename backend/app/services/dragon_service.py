...
# ⚠️ 定位（2026-09-10，KB-DEC-019）：这是**映射索引**（词 → 内核类别，供归类检索），不是行情规律。
#    ① 各列表是**人工枚举**：新题材词不会自动进表，会退化为"无法归因"——属已知边界，非缺陷；
#    ② 下面的"持续性倾向"数值来自**业界共识转述**，**未经本项目数据验证**，仅作展示倾向，
#       不得作为选股权重或跨题材外推依据；要升格须走 strategy_verify.py 的准入闸门。
THEME_CORE_RULES = [
...
def entry_checklist(
    *,
    ...
    # note 是对外文案，直接进 UI 纯文本渲染 → 不得出现 Markdown 标记（曾泄漏字面反引号
    # `missing`）。「未取到」那句只在确实有缺项时才追加：missing 为空时静态出现会让人
    # 以为有缺项（与三态纪律同名但语义相反，属误导）。
    note = (
        "以上为条件清单而非买卖建议：信号未同时满足就不构成介入理由；"
        "任一失效条件出现即推翻判断。价格区间需结合个股市值、题材阶段与自身仓位确定。"
    )
    if missing:
        note += "未取到的输入已在「未取到」中显式列出，对应维度按「未判定」呈现，不做中性假设。"
