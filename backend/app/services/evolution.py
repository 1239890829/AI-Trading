...
受限自治默认开启：应用内调度器会自行越过议程阶段，执行受白名单约束的参数实验与
知识沉淀；管理员可用正式环境变量一键停机。
...
- **受限自治默认**：`ASHARE_AGENT_AUTONOMY_ENABLED=1`；设为 0 → 只生成议程不执行（降级建议清单），
  **且调度器里的自动转正 / 自动回滚一并停止**（R10，2026-09-14：此前这两处位于
  自治判据之外，关掉开关仍会写库改状态——"停机"只覆盖了局部入口）。
  C 类还必须由管理员独立设置 `ASHARE_AGENT_CODE_CHANGE_ENABLED=1`；否则只允许建议、
  议程或 patch 预览，不得修改工作区、提交或合并。
