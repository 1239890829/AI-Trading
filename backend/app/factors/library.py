...
FACTORS: tuple[FactorDef, ...] = (
    ...
    # ---------------------------------------------------------------- 2026-09-14 RSH-003 批次
    # 来源：TA-Lib 标准指标**逐个转正**（账本 §6.0 `RSH-003`，MFI / OBV 优先，与 `fund_flow` 互验）。
    # 两者均**不照抄** TA-Lib 原式，各有 A 股适配口径，差异已逐条写进 note（不藏在注释里）。
    # 实测（2026-09-14 真实库 5562 只 / 2433 交易日）：两枚均 **PASS**，但彼此相关 0.97、
    # 且与既有 `sump20` 相关 0.95~0.96 ⇒ **族内冗余 ⇒ 登记留档、不重复计权**（族代表位仍 `mom20`）。
    FactorDef(
        "mfi20", "volume_price", 22,
        "mfi20_w",
        "TA-Lib MFI（20 日）资金流强度 = 100 − 100/(1 + 正流入/负流出)。"
        "**两处有意的口径差异（A 股适配）**：① 典型价 TP 用**复权**价"
        "（`((H+L+C)/3) × close_adj/close_price`）——照抄原价会让除权日（10 送 10）"
        "腰斩的 TP 被误读为「巨额资金流出」并污染随后 20 日；② 资金流用**成交额**"
        "（`turnover`）而非股数，以便与 `fund_flow` 口径互验。"
        "⚠️ 与 `vsumd20`（量的 RSI）**不是同一件事**：后者方向取自**量变化**"
        "（volume vs 前一日量），本因子取自**价格方向**（TP_adj vs 前值）；"
        ...
        "「资金流入强」在该尺度上是**反向**信号。窗口有效样本 <20 或 TP 全程持平 → NULL。",
    ),
    FactorDef(
        "obv20", "volume_price", 22,
        "obv20_w",
        "TA-Lib OBV（20 日**归一化**）= Σ sign(Δclose_adj)·volume / Σ volume ∈ [-1, 1]。"
        "**有意差异**：原式 OBV 是**路径依赖**累积量（依赖起点选取、跨股票不可比），"
        "直接入因子会把「从哪一天开始算」变成噪声，故取窗口内**净额占比**。"
        "与 `vsumd20` 同形而判据不同：后者按**量增减**，本因子按**价涨跌**"
