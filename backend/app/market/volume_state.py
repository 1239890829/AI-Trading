...
## 实证边界（KB-STOCK-32，2026-09-13）

10 年全 A 复权日 K 实测（994 万个股日）：**倒 U**——量比十分位次日收益在 D6 达峰（+0.074%），
**极端组 D10 = −0.127%**；vratio 连续截面 RankIC = **−0.0474（11/11 年度为负）**，通过本仓因子
门槛 ⇒「极端放量」是**稳定的负向截面信号**。消费方状态：`score_capital` 量比子评分**已按实证
改写为倒 U**（≥2.1 惩罚 / 0.9-1.65 峰区加分，2026-09-13）；`intraday_rules` 的
`BREAKOUT_VOLUME_RATIO=2.0` 是盘中触发器口径（与隔日口径不同），仍待 P2-33 专项验证。
