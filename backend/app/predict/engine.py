"""预判引擎：题材成立评分卡 + 梯队推演 + 介入计划（docs/strategy/theme-prediction.md §一/§三/§四）。
...
# 相位 → 环境分（S2-7 修正，2026-09-11）：键集必须与 `sentiment.engine.PHASE_ORDER`
# 全等。原表写着 **「启动」**——那是**题材阶段**不是市场相位，属两套概念混用：
# 死键「启动」永不命中，而真正的 **「修复」相位缺失** ⇒ 一直走 `.get(phase, 0.5)`
# 的默认 0.5，等于"修复期没有专门的环境分"这件事从来没被看见。
# 现把 0.5 显式写出来（**与修正前的实际生效值完全一致**，非口径变更），
# 并把「修复」的环境分变成可审阅、可调整的一项，而不是藏在默认值里。
PHASE_ENV_SCORE = {"冰点": 1.0, "修复": 0.5, "发酵": 0.7, "高潮": 0.3, "分歧": 0.9, "退潮": 0.2}
