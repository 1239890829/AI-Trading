"""停牌核查 / 异常波动风险评估（docs/halt-check-risk-analysis.md 第一批规则）。
...
- 普通异常波动（只发公告，**不停牌**）：3 日累计偏离值 主板 ±20% / ST ±12% /
  创业板科创板 ±30% / 北交所 ±40%
...
#: 第一批规则参数（经验初值，**未经回测校准**——见模块末尾 TODO）
...
# TODO(回测校准)：PENALTY_Y1 / PENALTY_Y3 / PENALTY_BOARDS 与 RED_DEV_10D 均为
# 经验初值。文档第 6 节要求用近 6 个月数据回测「排除红线票」对组合收益与最大
# 回撤的影响后再定稿。校准完成前这些数字只能算占位，不应据其做仓位决策。
