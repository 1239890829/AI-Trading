...
BREAKOUT_VOLUME_RATIO = 2.0     # 放量突破（%）——✅ 盘中口径已验证为「正选择」（2026-09-13 §6.21：
                                # TDX 5min 重演 15,301 次触发，vr≥2 组触发→收盘 +1.229% > 未确认
                                # +1.098% > 缩量 +0.761%，单调；KB-STOCK-32 附记③），常量保留。
                                # ⚠️ 与日线近似口径（T 收盘进）方向相反——执行时点是量能信号语义的一部分。
