        ...
        <div className="flex items-center gap-2 text-[11px]">
          {st && (
            <span className="text-zinc-400">
              已清算 {st.settled} · 胜率{" "}
              <span className={st.win_rate != null && st.win_rate >= 0.5 ? "text-up" : "text-down"}>
          ...
          <button
            ...
            className="rounded border border-zinc-200 px-1.5 py-0.5 text-zinc-400 hover:text-zinc-600 dark:border-zinc-700 dark:hover:text-zinc-200"
          >
            历史 {showHistory ? "▲" : "▼"}
          </button>
        </div>
            ...
            <tbody>
              {(data?.rows ?? []).map((r) => (
                <tr key={r.id} className="border-b border-zinc-100 align-top last:border-0 dark:border-zinc-800/60">
                  <td className="py-1.5 pr-2 font-mono text-[11px] text-zinc-400">{r.entry_time || "--"}</td>
                  <td className="py-1.5 pr-2">
                    <span className="font-medium text-zinc-800 dark:text-zinc-100">{r.name || r.symbol}</span>
                    <span className="ml-1 font-mono text-[10px] text-zinc-400">{r.symbol}</span>
                    {r.is_leader && (
                      <span className="ml-1 rounded bg-rose-500/10 px-1 text-[10px] text-rose-600 dark:text-rose-300" title="题材内最高连板（龙头识别）">
                        龙头
                      </span>
                    )}
                    {r.boards > 0 && <span className="ml-1 font-mono text-[10px] text-zinc-400">{r.boards}板</span>}
                  </td>
                  <td className="max-w-[8em] truncate py-1.5 pr-2 text-zinc-500" title={r.source_theme}>
                    {r.source_theme || "--"}
                  </td>
                  ...
                  <td className="py-1.5 pr-2">
                    {r.verdict === "success" && <span className="text-up">✓ 成功</span>}
                    {r.verdict === "fail" && <span className="text-down">✗ 失败</span>}
                    {r.verdict === "flat" && <span className="text-zinc-400">— 持平</span>}
                    {r.verdict === null && (
            ...
            <tbody>
              {(data?.history ?? []).map((h) => (
                <tr key={h.trade_date} className="border-t border-zinc-100 dark:border-zinc-800/60">
                  <td className="py-1 pr-2 font-mono text-zinc-500">{h.trade_date}</td>
                  <td className="py-1 pr-2 text-right font-mono">{h.stats.total}</td>
                  <td className="py-1 pr-2 text-right font-mono text-up">{h.stats.success}</td>
                  <td className="py-1 pr-2 text-right font-mono text-down">{h.stats.fail}</td>
