  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="grid shrink-0 grid-cols-2 gap-1 border-b border-zinc-100 px-3 py-2 text-xs dark:border-zinc-800/60">
        ...
        <span className={paper.acc.total_pnl >= 0 ? "text-up-ink dark:text-up" : "text-down-ink dark:text-down"}>
      ...
      <h3 className="shrink-0 px-3 pb-1 pt-2 text-xs font-medium text-zinc-600 dark:text-zinc-300">持仓</h3>
      <div className="min-h-0 flex-1 overflow-y-auto">
            ...
            {paper.positions.map((pos) => (
              <tr key={pos.symbol} className="border-b border-zinc-100 last:border-0 dark:border-zinc-800/60">
                <td className="px-3 py-1.5">
                  ...
                  <div className="text-[11px] text-zinc-600 dark:text-zinc-400">{pos.quantity}股 · 可卖{pos.available}</div>
                </td>
                <td className="px-2 py-1.5 text-right font-mono text-xs">{fmt(pos.cost_price)}</td>
                <td className={`px-3 py-1.5 text-right font-mono text-xs ${(pos.pnl ?? 0) > 0 ? "text-up-ink dark:text-up" : (pos.pnl ?? 0) < 0 ? "text-down-ink dark:text-down" : "text-zinc-600 dark:text-zinc-400"}`}>
                  {pos.pnl != null ? fmt(pos.pnl) : "--"}
                </td>
              </tr>
            ))}
            {paper.positions.length === 0 && (
              <tr><td colSpan={3} className="px-3 py-4 text-center text-xs text-zinc-600 dark:text-zinc-400">空仓</td></tr>
            )}
          </tbody>
        </table>
        {paper.orders.filter((o) => o.status === "pending").length > 0 && (
          <>
            <h3 className="px-3 pb-1 pt-2 text-xs font-medium text-zinc-600 dark:text-zinc-300">挂单</h3>
        ...
        <h3 className="shrink-0 px-3 pb-1 pt-2 text-xs font-medium text-zinc-600 dark:text-zinc-300">成交记录</h3>
        <table className="w-full text-xs">
          <thead>
            <tr className="text-[10px] text-zinc-600 dark:text-zinc-400">
          ...
          <tbody>
            {fills.map((f, i) => (
              <tr key={`${f.date}-${f.side}-${f.price}-${i}`} className="border-b border-zinc-100 last:border-0 dark:border-zinc-800/60">
                <td className="px-3 py-1 font-mono text-zinc-600 dark:text-zinc-400">{f.date || "--"}</td>
                <td className={`px-2 py-1 ${f.side === "buy" ? "text-up-ink dark:text-up" : "text-down-ink dark:text-down"}`}>{f.side === "buy" ? "买入" : "卖出"}</td>
                <td className="px-2 py-1 text-right font-mono">{fmt(f.price)}</td>
                <td className="px-2 py-1 text-right font-mono">{f.quantity}</td>
                <td className="px-3 py-1 text-right font-mono text-zinc-600 dark:text-zinc-400">{fmt(f.fee ?? 0)}</td>
              </tr>
            ))}
            {fills.length === 0 && (
              <tr><td colSpan={5} className="px-3 py-3 text-center text-zinc-600 dark:text-zinc-400">本股暂无成交</td></tr>
            )}
          </tbody>
        </table>
