...
export interface JevTraceAnswer {
...
export interface JevDecisionTrace {
  ...
  model: string | null;
  answers: Record<string, JevTraceAnswer>;
  latency_ms: number;
  reason: string | null;
}
...
export interface JevOverview {
  status: {
    state: "ready" | "disabled" | "not_configured" | string;
    model: string;
    ...
    metrics: {
      ...
      purposes: Record<string, Record<string, number>>;
      comparisons: Record<string, Record<string, number | null>>;
      routing: Record<string, Record<string, number | null>>;
    };
  };
  historical_usage: {
    calls: number;
  ...
  recent_decisions: JevDecisionTrace[];
  trace_scope: "runtime_only";
  privacy: string;
  fallbacks: Record<string, string>;
}

export async function getJevOverview(limit = 40): Promise<JevOverview> {
  return (await getJson<JevOverview>(`/api/agent/jev/overview?limit=${limit}`)).data;
}
