...
import { lineOf, maskComments, maskCommentsAndStrings } from "@/lib/src-mask";
...
// 注释/字符串掩码器与 `lineOf` 已抽到 `@/lib/src-mask`（2026-09-14 批次 B5 单点化）：
// 该掩码器现在同时被 `css-vars.test.ts` 使用，而写在测试文件里会让「测试互相
// import ⇒ 连带执行对方 describe ⇒ 用例重复注册」，故归位到非测试模块。
