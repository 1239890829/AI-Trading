...
/**
 * 与服务端 `READ_IDS_MAX` 一致：逐条已读表的**硬上界**。
 *
 * ## 已知边界（C-1，2026-09-12 评审批次 2 定性：挂账，不修）
 * `readIds` 是**无天然上界**的逐条登记表：`withRead` 会把每一条"晚于水位且被单独点开"
 * 的 id 追加进来（水位只在「全部已读」时推进）。因此**只要长期只用逐条已读、从不点
 * 「全部已读」**，它就会单调增长直到撞上这个 500 的上界；撞上后按「保留较新 500」截断，
 * 更早被单独读过的条目会**重新变回未读**（徽标 +1）。后果轻、不丢数据，故只挂账。
 *
 * **为什么不能用"折进水位数"来消除这个上界**：条目 id 是 `alert_triage` 去重用的
 * 事件标识，**不带时间戳**，从它推不出"读到哪一刻"；若强行把 id 对应的 ts 当水位，
 * 会把水位与它之间**从未被读**的条目一并吞掉（把未读擦成已读）——那比"少数条目回弹
 * 成未读"严重得多。要真正消除上界，得改登记结构（id 带 ts，或改成按条登记的水位），
 * 属**口径变更**，须前后端同改并复核 `merge_read_state` 的单调规则。
 *
 * ⚠️ 后人注意：**不要**把它当成 bug 去"顺手修"成水位——那会破坏单调性（见上）。
 */
