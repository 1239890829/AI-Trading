...
检查项（对应 §8.2）：
  ...
  C 超层配额     KB 分类文件 >800 行 / 框架·总纲 >500 / 报告 >600（已判定的容忍项见 TOLERATED）
...
def check_over_limit() -> list[tuple[str, int, int]]:
    ...
    # KB 分类文件：文件级 >800 才提示（条目级 ≤60 由人/议程核）
    for p in sorted((DOCS / "kb").glob("0[1-9]*.md")):
        n = len(_read(p).splitlines())
        if n > KB_FILE_CAP:
            out.append((f"docs/kb/{p.name}", n, KB_FILE_CAP))
    return out
...
def main() -> int:
    ...
    if not quiet:
        ...
        print(f"[INFO] C 已登记容忍项：{len(TOLERATED)} 份（{', '.join(TOLERATED)}）")
