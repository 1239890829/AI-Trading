...
description: Canvas 图表（lightweight-charts / ECharts 等）改动后的实际渲染验收工作流——模型读不了 PNG，用 agent-browser 文本通道 + 挂点 + 像素采样完成「以实际渲染为准」的验收。适用于分时/K线/热力图等 Canvas 绘制内容的 UI 验证。
read_when:
  - 改动了 Canvas 图表组件（分时图/K线图/热力图）需要验收
  - 需要验证图表纵轴范围、priceLine、序列颜色是否真的渲染
  - 需要验证「换主题后画布配色跟着换」且「图表未被重建」
  - agent-browser CLI 从 PATH 消失时
agent_created: true
---

# Canvas 图表验收工作流（agent-browser 文本通道 + 像素采样）

**背景**：模型读不了 PNG 截图（返回"不支持图片"）；Canvas 绘制内容无 DOM 文本，
snapshot 无障碍树拿不到图表内部。本 skill 提供「挂点读状态 + canvas 像素采样」
两件套，满足用户纪律「验收以实际渲染为准，不靠推理」。

## 0. 启动 agent-browser（CLI 常从 PATH 消失）

```bash
npx -y agent-browser open "http://localhost:3000/..."   # 使用当前环境可用版本，不依赖任何平台私有安装目录
```

- 后续命令均 `npx -y agent-browser <cmd>`；结束时必须 `npx -y agent-browser close`。
...
- 布局/间距类纯视觉问题文本通道仍看不出，如实告知用户"无法目视，需你确认"。
