...
3. **形成可行动集**：排除 `已完成/已退出/已合并`；`待条件` 未满足排除；`依赖` 中任一硬开工 ID 未完成则排除。`效果前置` 不阻止采样/工程开工，但阻止效果主张、策略/参数晋级和生产权重切换。
4. **确定当前主门**：在 G0–G4 中找最低的、仍存在可行动 `阻断` 任务的门。只要该门存在阻断项，默认禁止更高门成为主切片。
5. **门内排序**：先 `阻断` 后 `非阻断`；同角色按 P0 → P1 → P2 → `门内序` 升序。不得用“新任务更有趣/更近用户问题”覆盖这个顺序。
...
2. 网页审核/实施收口前必须确认当轮已有网页 `Preflight` 主动审计回执，并由网页对准确实现差异再形成 `Review` 回执：即使测试/CI 全绿，是否仍存在静默失败、部分提交、权限绕过、旧状态冻结、坏测试或治理漂移。Codex/作者自检不能自产 Review；缺 Review 或新发现会改变安全/正确性时，不得把该轮视为完整审核通过。
