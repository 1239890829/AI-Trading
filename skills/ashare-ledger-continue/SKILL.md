...
- 若当前问题跨多个模块、要求重大重构、长期机制取舍或 U48/GOV-027 生命周期复核，按需读取 `skills/living-system-governor/SKILL.md` 作为上层判断协议；**不要把它加入每轮固定读取链**，避免无关切片重复消耗上下文。它不改变 §3 的阶段门选刀算法，也不产生第二状态源。
...
4. 发现新的模型、工具链、架构、产品语义、协作或治理决策时，必须按 `docs/plan-registry.md` §1.1 做**传播核对**：列出受影响权威面已更新项和不适用项/理由。只更新专题蓝图不算完成。跨模块长期取舍可用 `living-system-governor` 的 Context Snapshot、竞争方案、falsifier 与 KEEP/FIX/MERGE/EXPERIMENT/WATCH/RETIRE 协议辅助审计，但最终调度与状态仍只认总账/stage。
