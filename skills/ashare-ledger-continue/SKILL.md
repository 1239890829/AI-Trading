...
- 若当前问题跨多个模块、要求重大重构、长期机制取舍、U48/GOV-027 生命周期复核，或 U49 主动审计发现跨模块高风险问题，按需读取 `skills/living-system-governor/SKILL.md` 作为上层判断协议；**不要把它加入每轮固定读取链**，避免无关切片重复消耗上下文。它不改变 §3 的阶段门选刀算法，也不产生第二状态源。
...
4. **确定当前主门**：先在 G0–G4 找最低的、仍存在可行动 `阻断` 任务的门；任一 blocker 存在时，所有 non-blocker 都不得抢占。若 G0–G4 已无任何可行动 blocker，则回落到最低的、仍有可行动 `非阻断` 任务的门；两类都没有时才报告“无普通主切片”。
5. **门内排序**：当前门候选包含该门全部可行动任务，先 `阻断` 后 `非阻断`；同角色按 P0 → P1 → P2 → `门内序` 升序。不得用“新任务更有趣/更近用户问题”覆盖这个顺序，也不得在 blocker 清零后因 selector 返回空值而停掉尚未完成的 non-blocker。
6. **GX 只伴随**：`持续治理` 任务最多作为一个不冲突伴随切片；不能因文档、雷达、CI优化而跳过业务阶段门。
7. **G5 只验收**：只有当前准确候选已完成实现/研究且到发布/验收时才进入；不能把验收任务提前当开发任务。
...
6. Jev 只用于 bounded 语义判断/验证或条件能力路由；确定性金融规则、权限、撮合、风控、时间/数值计算和真实执行不得委托。human gold 未完成前不宣称 accuracy、不调生产阈值。
7. 完成后按 `ashare-task-handoff` 回填同一阶段任务、证据与 handoff 指针，提交/推送并核到远端；未推送不说已交付。
8. 本地验证、网页独立审核、准确版本 CI 和运行加载分开。额度不足不试探、不换付费 API；平台安全拒绝不绕过。
