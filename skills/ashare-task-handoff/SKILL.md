...
**U49 主动审计回执（每轮必填）**：交接必须记录触发点、审计范围、八类检查面（自锁/事实源/顺序与部分失败/幂等去重与 unknown/权限 fail-open/动态状态冻结/测试盲区/治理传播）、发现项分级、未证未知、owner，以及它是否改变当前验收、阶段门或下一刀。没有发现也必须写“检查范围 + 未发现新阻断项”；没有这份回执，不得把该轮称为完整审核/收口。主动审计本身不是第二个业务切片，也不授权跨门。
