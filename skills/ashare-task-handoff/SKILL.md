...
8. 编号不复用；退出/合并有依据和去向。下一轮始终先从最新 `master` 调用 `ashare-ledger-continue`，先算最低未闭环主门，再按角色/P0-P2/门内序选一刀。handoff 允许实施时，用户说“继续任务/继续”只授权该一刀；若要跨门，必须已有网页写入的 `CROSS_GATE_EXCEPTION`，Skill/Codex 不得自行创建。
