...
- For dependency security fixes, keep `package.json` and `npm-shrinkwrap.json` aligned, prefer the smallest compatible patch set, reinstall with `npm ci --ignore-scripts`, run `npm run verify`, and audit against the official npm registry (`npm audit --registry=https://registry.npmjs.org`). Do not use `npm audit fix --force`.
